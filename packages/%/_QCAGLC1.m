...
;%QCAGLC1^INT^1^59547,73875^0
%QCAGLC1(S,D) ;;02:36 PM  4 Sep 1996
 
 
 D COP(S,"",D,0) Q
COP(S,N,D,V) N I,AD,ADD,S1,B,C,DIR
 S S1=$$CONCAT(S,N),DIR="^" S:$E(S,2,2)="[" DIR=$P(S,"]",1)_"]"
 S:$L(DIR,"[")=3 DIR=$P(S,"]",1,2)_"]"
 ;;S AD=DIR_$E($Q(@S1),2,600)
 S AD=DIR_$E($$^%Q($Q(@S1)),2,600)   ;; cao debug
 S ADD=$$CONCAT(D,$E(AD,$L(S)+1,$L(AD)-1))
 F I=1:0 Q:AD'[$E(S,1,$L(S)-1)  S @ADD=@AD W:V !,ADD,?40,@(ADD)," <-- ",?50,AD,?90,@(AD) S AD=DIR_$E($$^%Q($Q(@AD)),2,600),ADD=$$CONCAT(D,$E(AD,$L(S)+1,$L(AD)-1)) ;; cao debug
 I ($D(@S)#10)=1 S @D=@S
 Q
 
COP1 S @ADD=@AD W:V !,ADD,?40,@(ADD)," <-- ",?50,AD,?90,@(AD) S AD=DIR_$E($Q(@AD),2,600),ADD=$$CONCAT(D,$E(AD,$L(S)+1,$L(AD)-1))
 S @ADD=@AD W:V !,ADD,?40,@(ADD)," <-- ",?50,AD,?90,@(AD)
 S AD=DIR_$E($Q(@AD),2,600)
