...
;%QCAGTU1^INT^1^59547,73875^0
TREEWAY ;
 
 K B,NOM S %IFLAG=0,STOP=0,POSI=1,NVIRG=0,NAMTAB=$S(NOMTREE["(":NOMTREE_",",1:NOMTREE),%8DEB=1 F Z1=0:0 S POSI=$F(NAMTAB,",",POSI) Q:POSI=0  S NVIRG=NVIRG+1
 S AB="B",(%PR,%8DEB)=NVIRG+1,%PR=NVIRG
 ...
 K %IFLAG,%APAR,NAMTAB,POSI,Z1,Z2
L S B(%PR)=$S($D(B(%PR)):B(%PR),1:-1),V=B(%PR),VP=""""_V_"""",C=NOM_VP_")",(E,B(%PR))=$N(@C) G:B(%PR)=-1 S S NOM(%PR)=NOM,NOM=NOM_""""_B(%PR)_""",",%TREFC=%TREFC_$S(%TREFC'="":",",1:"")_B(%PR)
...
S I %PR=(NVIRG+1) S %IFLAG=-2 G FIN
 ...
 S:%PR'=1 %PR=%PR-1 S AB="B",%8DEB=%PR,NOM=NOM(%PR),%TREFC=$E(%TREFC,1,($L(%TREFC)-$L($P(%TREFC,",",%PR)))-1) G L
 Q
C3 G L
FIN K %8DEB,%APAR,%IFLAG,%PR,%TREFC,C,NAMTAB,POSI,VP,Z1,Z2 Q
TEST W !,"nom du global : " R NOMTREE G:NOMTREE="" FIN S ADR="ECR^%QCAGTU1" D ^%QCAGTU1 G TEST
ECR W !,NOM," %PR=",%PR Q
