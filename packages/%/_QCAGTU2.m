...
TREEWAY2 ;
 ...
 S NOM=$S(NOMTREE["(":NOMTREE_")",1:NOMTREE) I ($D(@NOM)=1)!($D(@NOM)=11),$L(@NOM)'=0 D @ADR G:STOP=1 FIN
 S %PR=%PR+1,NOM=$S(NOMTREE["(":NOMTREE_",",1:NOMTREE_"(")
L S @AB@(%PR)=$S($D(@AB@(%PR)):@AB@(%PR),1:-1),V=@AB@(%PR),VP=""""_V_"""",C=NOM_VP_")",(E,@"B"@(%PR))=$N(@C) G:@"B"@(%PR)=-1 S S NOM(%PR)=NOM,NOM=NOM_""""_B(%PR)_""","
FIRST S %1DATA=1,NOM=$E(NOM,1,$L(NOM)-1)_")"
 I ($D(@NOM)'=1)&($D(@NOM)'=11) S %1DATA=0
 D @ADR G:STOP=1 FIN I STOP=0 S %PR=%PR+1,NOM=$E(NOM,1,$L(NOM)-1)_"," G C3
 I STOP="F" S NOM=NOM(%PR),STOP=0 G C3
...
C3 G L
FIN Q
TEST W !,"nom du global : " R NOMTREE G:NOMTREE="" FIN S ADR="ECR^%QCAGTU2" D ^%QCAGTU2 G TEST
ECR W !,NOM,!,%1DATA,?20,"%PR=",%PR,?45,%TREFC,?60,B(%PR) W:%1DATA !,?20,"Valeur ===> ",@(NOM) Q
