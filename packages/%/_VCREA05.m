...
;%VCREA05^INT^1^59547,74034^0
CREAVAL ;
 K ^POSENR($I) S NEXT=-1,PAGE=1,LI=5,LILIM=22
EXP F %U=1:1 S NEXT=$N(LIBEL(NEXT)) Q:NEXT=-1  S ^POSENR($I,LI)=NEXT,LI=LI+1
FEXP D CLEPAG^%VVIDEO
 I $D(SPMAHC) S DX=0,DY=0 D BLD^%VVIDEO X XY W "champ ",NCH D NORM^%VVIDEO
 S DY=-1,DX=2 F %U=1:1 S DY=$N(^POSENR($I,DY)) Q:DY=-1  X XY W ^POSENR($I,DY),?30,"= ",CHAR(LIBEL(^POSENR($I,DY)))
