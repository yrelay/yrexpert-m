...
;%VDUPGRI^INT^1^59547,74034^0
GEDUPGRI ;
 G:'($D(^%SCRE(SCR,"RELATION"))) FIN D POCLEPA^%VVIDEO W "Un instant..."
 S %MOD=0 K ^PILMOD($J) S %V=^%SCRE(SCR) F %U=1:1:%V I ($D(^%SCRE(SCR,%U))#10)=1 S:RESUL(%U)'=OR(%U) %MOD=1 S ^PILMOD($J,SCR,%U)=1
 G:'(%MOD) FIN
...
LOOP G:STOP<0 KVAL S %V=-1 F %U=1:1 S %V=$N(VAL(%V)) Q:%V=-1  G:VAL(%V)'=RESUL(%V) KVAL
 D DUPL^%VCMSCOR G:'(OK) LOOP
 K OR S %U=-1 F %V=1:1 S %U=$N(^PILMOD($J,SCR,%U)) Q:%U=-1  S OR(%U)=AFRESU(%U)
