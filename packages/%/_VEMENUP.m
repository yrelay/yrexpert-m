...
GEMENUPG(%T,AD,CONT) 
 ...
 D VIDEO^%VMENVID K DEF S AD="",CONT="",T1="^GM4($I,PG)",T2="^GM2($I,PG)",T3="^GM1($I,PG)",T4="^GM3($I,PG)",T5="^GM5($I,PG)",T6="^GM6($I,PG)",T7="^GM7($I,PG)" F %U=T1,T2,T3,T4,T5,T6,T7 I '($D(@%U)) D ^%VMENGEN Q
...
AFM2 
 ...
 S OP=$S(($D(@MES)#10)=1:$P(@MES,"^",4),1:0) Q:'(OP!NUM)  S GLO=$S(OP:"^GM5($I,PG,IPG)",1:"^GM7($I,PG,IPG)")
 S LC=-1 F %U=0:0 S LC=$N(@GLO@(LC)) Q:LC=-1  S CC=-1 F %U=0:0 S CC=$N(@GLO@(LC,CC)) Q:CC=-1  S OP=@GLO@(LC,CC),DY=LC,DX=(CC-$L(OP))-1 X XY,AD2
 K AD1,AD2,IPG Q
 
 
 
SUI D EFBM S:($D(^GM1($I,PG))#10)=1 PX=$P(^GM1($I,PG),"^",2),PY=$P(^GM1($I,PG),"^",3)
 G ^%VMENLEC
