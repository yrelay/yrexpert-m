...
CONF(CONFIRM,AD,CONT) 
 
 
CONF2 N %A1,%B,%EQI,%U,%XY,%Y,CHP,CP,DX,DY,EQ,LIB,PX,PY,T1,T2,T3,T4,T5,T6,X1,X2,X3,XC,YC
 S AD="",CONT="",T1="^GF4($J)",T2="^GF2($J)",T3="^GF1($J)",T4="^GF3($J)",T5="^GF5($J)",T6="^GF6($J)",TBMOV=T2,PY=$O(@T2@(""))
 O $I S PX=$O(@T2@(PY,"")),DY=-1 F %U=1:1 S DY=$N(@T1@(DY)) Q:DY=-1  S DX=PX X XY S LIB=@T1@(DY) D WRIT
 S %EQI=^GF6($J) D ^%VCRIFLD($J("",$P(%EQI,"^",4)),$P(%EQI,"^",2)+1,$P(%EQI,"^",1),$P(%EQI,"^",2)+1,$P(%EQI,"^",3)-1)
 
LEC S CP=0
LEC1 S DX=PX,DY=PY,T5=T6 X XY D ^%VMOVREV S PX=$X,PY=$Y G GET:X1=13,CTRL:X1'>26
 G:CONFIRM ERRLEC
 G CHAIN:X1=47 S X1=$C(X1) G:$D(@T3@(X1)) EX S:'($D(RM)) RM=80 O 0
...
FIN S:'($D(RM)) RM=80 O 0
 K %Z,%EQI,EQ,LGI,LMX,CC Q
 
REV(AY,AX) 
 Q:'($D(%MC))  S DX=$P(%MC,"^",1),DY=$P(%MC,"^",2),LIB=$P(%MC,"^",3,999) X XY D WRIT S DX=AX,DY=AY D BLD^%VVIDEO,UDL^%VVIDEO X XY W $TR(^GF2($J,DY,DX),"#","") X XY D NORM^%VVIDEO
 Q
 
COM(AY,AX) 
 S EQ=^GF6($J,AY,AX),%EQI=^GF6($J)
 D ^%VCRIFLD($J("",$P(%EQI,"^",4)),$P(%EQI,"^",2)+1,$P(%EQI,"^",1),$P(%EQI,"^",2),$P(%EQI,"^",3))
 D ^%VCRIFLD(EQ,$P(%EQI,"^",2)+1,$P(%EQI,"^",1),$P(%EQI,"^",2),$P(%EQI,"^",3)) S $P(^GF6($J),"^",4)=$L(EQ) Q
