...
LOP3 S DEP=0,C=$$^%QZCHW(^%SCRE(SCR,ICC,1)),DX=^%SCRE(SCR,ICC,2),DY=^%SCRE(SCR,ICC,3) X XY I $D(^%SCRE(SCR,ICC,"MODAFF")) S MOD=^%SCRE(SCR,ICC,"MODAFF") D @MOD X XY
 ...
 D GET^%VLECFL2(RESUL(ICC),%XI,%YI,LXG,LXD,.CTRLF,.Y1) S:CTRLF="F" CTRLF=1 S:(CTRLF="K")&(%AJM="C") X1=1 S:(CTRLF=1)!(CTRLF="L") RESUL(ICC)=Y1 Q:(CTRLF=1)!(CTRLF="L")  G VDP:CTRLF="K",ABEND:CTRLF="A"
 I CTRLF'="" D ^%VCRIFLD($J("",$S($L(Y1)>$L(RESUL(ICC)):$L(Y1),1:$L(RESUL(ICC)))),%XI,%YI,LXG,LXD)
 G ESCMV:$E(CTRLF)="*" I CTRLF'="" D CTRL G LOP3
