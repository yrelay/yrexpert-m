...
;%VTFLD^INT^1^59547,74036^0
GETFLD ;
 
 
 
 S DX=$X S:'($D(YG)) YG=""
 S PCX=^%SCRE(SCR,ICC,2)+$L(^%SCRE(SCR,ICC,1)),PCY=$Y
 U 0
LEC R *X1:^TOZE($I,"ATTENTE") S:X1=-1 X1=1
 ;;MODTM
 ...
 ;;FINNONDTM
 G HELP:X1=8,IMPR:X1=10,GOMME:X1=7,MFLD:X1=9,FIN:(X1=13)!(X1<2),CTRLF:X1=6,REFUS:X1<32
 W $C(X1) S LPG=((($Y-PCY)*80)+$X)-PCX,PG=$E(YG,1,LPG-1),PD=$E(YG,LPG+1,299),YG=PG_$C(X1)_PD
 G LEC
DEL I (($X-1)<PCX)&(PCY=$Y) G REFUS
...
MFLD S ADX=DX,DFLD=PCX,FLD=YG,XEW=$X,YEW=$Y D POCLEPA^%VVIDEO,BLD^%VVIDEO,REV^%VVIDEO,BLK^%VVIDEO W " Mode extension " D NORM^%VVIDEO S DX=XEW,DY=YEW X XY
 ...
 D ^MODIFLD S XEW=$X,YEW=$Y D POCLEPA^%VVIDEO S DX=XEW,DY=YEW X XY S CCX=$X I ABENDFLD=0 G MS
...
REFR S OICC=ICC,RESUL(ICC)=YG,ADX=$X,ADY=^%SCRE(SCR,ICC,3) D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR S DX=ADX,DY=ADY,ICC=OICC X XY G LEC
IMPR S ADX=$X,ADY=$Y D POCLEPA^%VVIDEO W "Impression en cours..." D ^%VIMPSCR,POCLEPA^%VVIDEO S DX=ADX,DY=ADY X XY G LEC
...
TEST D CLEPAG^%VVIDEO S YG="",%LIXG=30,%LIXD=50,%LIYH=5,%LIYB=15,SCR="TEST",ICC=1,DX=^%SCRE(SCR,ICC,2),DY=^%SCRE(SCR,ICC,3) X XY W ^%SCRE(SCR,ICC,1) D ^%VTFLD Q
