...
;JALTO2^INT^1^59547,74867^0
JALTOUT(LISTE,RDEL,VIOL,FOCAP,DATE,DATELIM,SENS,MODEX,MAJDEL) 
 ...
 Q:'($D(@ADR))
 G GO
ADR(ADR,RDEL,VIOL,FOCAP,DATE,DATELIM,SENS,MODEX,MAJDEL) 
...
GO2 
 ...
 D JAMT^JALAC2(ARC,DTH,QU,DATLIM,VIOL,RDEL,SENS,FOCAP,MODEX,MAJDEL,.MPO,.R,LRES) K MPO
 
 G:(VISU=0)!(R=0) GO3
 S NER=@LRES+1,@LRES@(NER)=" ",NER=NER+1
 S @LRES@(NER)=ARC,NER=NER+1
 S @LRES@(NER)=$$^%QZCHW($S(R=1:"pas de composant",R=2:"pas d'operation",R=3:"machine indefinie",R=4:"duree fabrication inconnue",R=5:"operation impossible a placer",6:"regime calendaire mal compile",1:"donnees incompletes"))
 S @LRES=NER
GO3 D:VISU ^%VZEAVT("")
 Q
...
CADRE D CLEPAG^%VVIDEO,CARSP^%VVIDEO(0,1,17,80,1),CUROF^%VVIDEO,BLK^%VVIDEO,BLD^%VVIDEO,POCLEPA^%VVIDEO W "Jalonnement en cours "_$$NORM^%VVIDEO1_"CTRLA pour arreter"
 Q
