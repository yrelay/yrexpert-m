...
;SOURCHSF^INT^1^59547,74868^0
SOURCHSF ;
 
 S YA=$$^%QZCHW("SOURIS") K ^V($I,YA) S CV="^V($I,YA)",@CV@($$^%QZCHW("NOMBRE.DE.DENTS"))=%NBDTS,@CV@($$^%QZCHW("DIRECTION.SOURIS"))=DIR(DIR),DIR="?",%ABEND=0 D ^SOURGFLA
...
RBLI I PARAP="ARRET",VPA=1 S %ABEND=1 G LOPAR
 G LOPAR
FIN I $D(^V($I,YA,$$^%QZCHW("PROCHAINE.DIRECTION"))) S DIR=DIR(^V($I,YA,$$^%QZCHW("PROCHAINE.DIRECTION")))
 K UNDEFCI Q
AVERTI D POCLEPA^%VVIDEO,^%VSQUEAK W "* ",PARA,$$^%QZCHW(" * non defini, nous aurons peut-etre des problemes") H 3 Q
SIFON S SIF=0 Q:@AFFECT'["$"  S CONTAF=@AFFECT Q:CONTAF'["$$"  S SIF=1,ROUTI="^"_$P($P(CONTAF,"/",1),"$$",2),EXPFONC=$S($E(CONTAF,1)="(":$E(CONTAF,2,$L(CONTAF)-1),1:CONTAF) D @ROUTI S AFFECT="%RESUL",%RESUL="%RESULTAT" K CONTAF,EXPFONC,ROUTI Q
...
VERIFDIR 
 B  D POCLEPA^%VVIDEO W $$^%QZCHW("je choisis donc la direction "),^V($I,YA,$$^%QZCHW("PROCHAINE.DIRECTION")) H 1 Q
