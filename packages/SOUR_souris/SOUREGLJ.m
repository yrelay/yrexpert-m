...
AFF D CLETEX^%VVIDEO S DX=0,DY=6 D CLEBAS^%VVIDEO S DY=6,MSG=$$^%QZCHW("Page ")_PG_"/"_NBPG,DX=40-($L(MSG)\2) X XY W MSG S LI=-1 F %U=1:1 S LI=$N(^PAGE($I,PG,LI)) Q:LI=-1  S DY=LI,DX=5 X XY W ^PAGE($I,PG,LI)
 Q
REG ;;      La souris tres affamee doit trouver le chemin le plus court afin
 ;;de s'offrir le fromage pour son dejeuner.
 ;;      Ce rongeur est tres peu, voire, pas du tout intelligent. Il se
 ;;dirigera d'une facon totalement aleatoire pour devorer son mets, Sans 
 ;;passer deux fois au meme endroit, sauf s'il est dans une impasse. Face
 ;;a ce probleme et pour venir en aide a nos amis les betes, nous decida-
 ;;mes d'alleger sa peine en mettant a la disposition de cette espece 
 ;;animale fort sympatique un outil performant dans le monde industriel :
 ;;un systeme a base de connaissance appele : Systeme Expert.
 ;;      Vous pouvez aider la souris a se deplacer intelligemment dans le 
 ;;labyrinthe. Grace a votre savoir-faire et a votre perspicacite, se
 ;;manifestera ou non l'agressivite intellectuelle du petit rongeur.
 ;;
 ...
 ;;plus ou moins attenuer cette propagation selon leur degre de 
 ;;permeabilite. Plus ce degre est eleve, plus l'odeur traversera l'obsta-
 ...
 ;;savoir-faire le necessite.
 ;;      Ces parametres predefinis (mots reserves) sont :
 ;;
 ...
 ;;          Yrelay vous souhaite un bon divertissement 
 ;;et bon appetit a la souris, si grace a "l'intelligence Artificielle"
 ;;que vous allez lui donner elle ne meurt pas de faim dans les meandres
 ;;du labyrinthe.
 ;;*
