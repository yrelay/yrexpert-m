...
;TO31STOC^INT^1^59547,74869^0
TO31STOC ;
 
 S:'($D(^TOZE($I,"FULL"))) ^TOZE($I,"FULL")=0 I ^TOZE($I,"FULL")=1 G FULL
 I $D(^TABIDENT(WHOIS,"INTERDIT","MOD",$I)) G FULL
 D AUTOST(.NOMTAB,.SCR,.RESUL) S %NFU="",%NF1=$P(NOMTAB,"]",2) S:%NF1="" %NF1=$P(NOMTAB,"^",2) I $D(^NAMEFUL(%NF1)) S %NFU="^[QUI]"_$P(^NAMEFUL(%NF1),"^",1)
...
AUTOST(NOMTAB,SCR,RESUL) Q:('($D(NOMTAB)))!('($D(SCR)))
 ...
 S IN=""""_LIEN_"""",RS="" I SCR="VALITOLE" S RS="VALI"_RESUL(1) F UU=2:1:4 S RS=RS_"/"_RESUL(UU)
 F IC=1:1 Q:'($D(RESUL(IC)))  S RESUL(IC)=$$GETLBLAN^%VTLBLAN(RESUL(IC)),ISN=$S($D(^%SCRE(SCR,IC))=11:"IND",1:"RES") D @ISN G:ABENDSCR=1 FIN
 S NOM=NOMTAB_"("_IN_")" G LONG:$L(NOM)>122,REFINS:IN="" S:$D(@NOM) %EXIST=1 S @NOM=RS
FIN Q
...
REFUL D ^%VSQUEAK,POCLEPA^%VVIDEO W $$^%QZCHW("Informations trop abondantes : veuillez abreger les champs ") H 2
 ...
 H 2 D ^%VAFIGRI,^%VAFISCR S IC=1,ABENDSCR=0 D ^%VKMODIS G TO31STOC:ABENDSCR=0 Q
REFUS D ^%VSQUEAK,POCLEPA^%VVIDEO W $$^%QZCHW("Il faut obligatoirement une information dans  "),^%SCRE(SCR,IC,1) H 2 D ^%VAFIGRI,^%VAFISCR S ABENDSCR=0 D LOOP^%VKMODIS G TO31STOC:ABENDSCR=0 Q
LONG D ^TOLONST2 Q
GTSFRESU(RESUL,FRESU) 
 D ^%VTFRESU Q
FULL D ^%VSQUEAK,POCLEPA^%VVIDEO W $$^%QZCHW("Stockage refuse...") H 2 Q
