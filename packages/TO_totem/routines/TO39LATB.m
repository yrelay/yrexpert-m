...
PCHSST I $D(^[QUI]FULL(YART,PCH)) G SUI
 D ^%ABLATIN(YART,PCH,.OK) G SUI:OK
 S ^[QUI]UNKN(WB1,PCH_"'"_YAL,$E(PCH,1,2)_$R(10000))=ADEL_$$^%QZCHW(" ")_FFF,^[QUI]REFLATER(WB1,YAL,PCH)=1,PCHCOMP=PCH_"'"_YAL D DEB^TO39KEMP S %BIL=PCH_$$^%QZCHW("(de ")_YAL_")",%PYT="UP" D ^%ABLIBTY S %BIL=YAL_"("_PCH_$$^%QZCHW(" de )"),%PYT=$$^%QZCHW("O") D ^%ABLIBTY
 I %TRAC=1 S VXPX="^V($I,"""_YAL_""","""_PCH_""")" G END
 S AEXEC=0 G END
SUI S %RL=1
FIN S CONCUR=1,%RL=1,VXPX="^V($I,"""_YAL_""","""_PCH_""")",^[QUI]REFLATER(WB1,YAL,PCH)=1,PCHCOMP=PCH_"'"_YAL D ^TO39KEMP S %BIL=PCH_$$^%QZCHW("(de ")_YAL_")",%PYT=$$^%QZCHW("P") D ^%ABLIBTY S %BIL=YAL_"("_PCH_$$^%QZCHW(" de)"),%PYT=$$^%QZCHW("O") D ^%ABLIBTY G END
RECON 
...
SUIT I $D(^[QUI]CONSTANT(YAL,PCH)) S VXPX="^[QUI]CONSTANT("""_YAL_""","""_PCH_""")",IMPCTE=1,%BIL=PCH_$$^%QZCHW(" de ")_YAL,%PYT=$$^%QZCHW("C") D ^%ABLIBTY G END
 I YAL=$$^%QZCHW("MATIERE") S VXPX=$$^%QZCHW("^M($I,^V($I,YA,""MATIERE""),""")_PCH_""")",IMPCTE=1,PCHCOMP=YAL,CONCUR=1 D ^TO39KEMP S %BIL=PCH_$$^%QZCHW("(de MATIERE)"),%PYT=$$^%QZCHW("P") D ^%ABLIBTY S YAL=WB1,PCH=$$^%QZCHW("MATIERE") D ^%AB3IMP2
 I YAL=$$^%QZCHW("%MACHINE") S VXPX=$$^%QZCHW("^S($I,^V($I,YA,""%MACHINE""),""")_PCH_""")",IMPCTE=1,PCHCOMP=YAL,CONCUR=1 D ^TO39KEMP S %BIL=PCH_$$^%QZCHW("(de %MACHINE)"),%PYT=$$^%QZCHW("P") D ^%ABLIBTY S YAL=WB1,PCH=$$^%QZCHW("%MACHINE")
 ...
 I YAL=$$^%QZCHW("PERE") S VXPX=$$^%QZCHW("^V($I,^V($I,YA,""PERE""),""")_PCH_""")",YAL=$$^%QZCHW("PERE"),IMPCTE=1,PCHCOMP=YAL,CONCUR=1 D ^TO39KEMP S %BIL=PCH_$$^%QZCHW("(de PERE)"),%PYT=$$^%QZCHW("P"),YAL=WB1,PCH=$$^%QZCHW("PERE") D ^%ABLIBTY,^%AB3IMP2 G END
 Q
END S YART=YARTP Q
UND Q
