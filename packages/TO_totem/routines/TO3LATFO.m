...
ABREV S PCHP=$P(PCH,"'",1),YAL=$P(PCH,"'",2),PCH=PCHP,YARTP=YART,YART=YAL D RECON G:IMPCTE=1 END S COMP=YAL D ^%ABSIDEN S YART=COMPID I '($D(^[QUI]FULL(COMPID))) S OARTI=ARTI,ARTI=COMPID D ^%ABPARAR S ARTI=OARTI K OARTI
 ...
 S ^[QUI]UNKN(WB1,PCH_"'"_YAL,$E(PCH,1,2)_$R(10000))=ADEL_" "_FFF,^[QUI]REFLATER(WB1,YAL,PCH)=1,PCHCOMP=PCH_"'"_YAL D DEB^%AB3KEMP,^%AB3IMP2 S %BIL=PCH_$$^%QZCHW("(de ")_YAL_")",%PYT="UP" D ^%ABLIBTY S %BIL=YAL_"("_PCH_$$^%QZCHW(" de)"),%PYT=$$^%QZCHW("O") D ^%ABLIBTY
 I %TRAC=1 S VPOL(XPX)="^V($I,"""_YAL_""","""_PCH_""")",XPX=XPX+1 G END
 S AEXEC=0 G END
SUI S %RL=1
FIN S %RL=1,VPOL(XPX)="^V($I,"""_YAL_""","""_PCH_""")",XPX=XPX+1,^[QUI]REFLATER(WB1,YAL,PCH)=1,PCHCOMP=PCH D ^%AB3IMPI S PCHCOMP=PCH_"'"_YAL D ^%AB3KEMP S %BIL=PCH_$$^%QZCHW("(de ")_YAL_")",%PYT="P" D ^%ABLIBTY S %BIL=YAL_"("_PCH_$$^%QZCHW(" de)"),%PYT=$$^%QZCHW("O") D ^%ABLIBTY
 G END
RECON 
...
SUIT I $D(^[QUI]CONSTANT(YAL,PCH)) S VPOL(XPX)="^[QUI]CONSTANT("""_YAL_""","""_PCH_""")",IMPCTE=1,XPX=XPX+1,%BIL=PCH_$$^%QZCHW(" de ")_YAL,%PYT=$$^%QZCHW("C") D ^%ABLIBTY G END
 I YAL=$$^%QZCHW("MATIERE") S VPOL(XPX)=$$^%QZCHW("^M($I,^V($I,YA,""MATIERE""),""")_PCH_""")",%BIL=PCH_$$^%QZCHW("(de MATIERE)"),%PYT="P" D ^%ABLIBTY S IMPCTE=1,XPX=XPX+1,YAL=WB1,PCH=$$^%QZCHW("MATIERE") D ^%AB3IMP2
 I YAL=$$^%QZCHW("%MACHINE") S VPOL(XPX)=$$^%QZCHW("^S($I,^V($I,YA,""%MACHINE""),""")_PCH_""")",%BIL=PCH_$$^%QZCHW("(de %MACHINE)"),%PYT="P" D ^%ABLIBTY S IMPCTE=1,XPX=XPX+1,YAL=WB1,PCH=$$^%QZCHW("%MACHINE") D ^%AB3IMP2
