...
;TO4COR^INT^1^59547,74871^0
TO4COR ;
 
LOOP K VALCLE S ADR="LOOP1^TO4COR",NOMTAB="^[QUI]TOTEXPER",VALCLE(1)=%EXPERT,VALCLE(2)=RESUL(1) D ^%VKMULG G:STOP'=1 CHAN2 Q
LOOP1 S NUM=-1 F UU=0:0 S NUM=$N(^TO4GEXPL($I,NUM)) Q:NUM=-1  S RESUL(NUM+%DRES)=$P(%RS(1),"/",NUM+1)
 S RESUL(1)=B2,RESUL(2)=%RS(2) D ^%VAFIGRI,^%VAFISCR
...
D S OLDNOM=NOM,ABENDSCR=0 D POCLEPA^%VVIDEO,^%VSQUEAK W $$^%QZCHW("N'oubliez pas de changer les champs cles") H 2 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^TO4MODIS G LOOP1:ABENDSCR=1 D ^TO4STOSH G LOOP1
M S OLDNOM=NOMTAB_"("""_B1_""","""_B2_""")",ABENDSCR=0 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^TO4MODIS G LOOP1:ABENDSCR=1 K @(OLDNOM) D ^TO4STOSH G LOOP11
