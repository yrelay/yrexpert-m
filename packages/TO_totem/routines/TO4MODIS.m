...
;TO4MODIS^INT^1^59547,74871^0
TO4MODIS ;
 
 S IC=1,ABENDSCR=0
LOOP G FIN:'($D(^%SCRE(SCR,IC))) D POCLEPA^%VVIDEO W:IC>%DRES $S($D(^TO4GEXPL($I,IC-%DRES)):^TO4GEXPL($I,IC-%DRES),1:" ??? ") S (PCX,DX)=$L(^%SCRE(SCR,IC,1))+^%SCRE(SCR,IC,2),DY=^%SCRE(SCR,IC,3) X XY
 S (OY1,Y1)=RESUL(IC)
READ S RET="READ" O 0 R *X1
 ;;DTM
 G:$$FLECHE^%INCCLAV(X1,.X1,.X2,.X3) RFLD2
...
272 R *X3
 ...
 G:X3="~" BELL
 G 272
RFLD2 G FIN:(X2=27)!(X3=27) I (((X1'=27)!(X2'=91))!(X3'>64))!(X3'<69) G BELL
 D @X3,TEST G LOOP
BELL D ^%VSQUEAK G LOOP
FIN O 0 Q
65 S DYC=$Y,DYV=$O(^IGCURGRI($I,SCR,-(DYC))) I DYV="" D ^%VSQUEAK Q
 S DXV=$N(^GCURGRIL($I,SCR,-(DYV),-1)),IC=^GCURGRIL($I,SCR,-(DYV),DXV) Q
66 S DYC=$Y,DYV=$O(^GCURGRIL($I,SCR,DYC)) I DYV="" D ^%VSQUEAK Q
 S DXV=$N(^GCURGRIL($I,SCR,DYV,-1)),IC=^GCURGRIL($I,SCR,DYV,DXV) Q
67 S IC=IC+1 Q
68 S IC=IC-1 Q
...
CHAMP D:(X1=7)!(YG=7) GOMME S ICC=IC,LL=$L(RESUL(ICC))
 ...
 S REFUSE=""
 S STOPUC=0 D VERIF G REFULOP:REFUSE=1 I $D(CMS),CMS'=1,$D(^[QU]SCRC(SCR,ICC,4)),^[QU]SCRC(SCR,ICC,4)'="" F UVU=1:1 S UCONT=$P(^[QU]SCRC(SCR,ICC,4),",",UVU) Q:UCONT=""  S IC=ICC D @UCONT G REFULOP:REFUSE=1 Q:STOPUC=1
 S IC=IC+1 I XXYYZZ'="",$L(Y1)=0 S PADMESSA=$$^%QZCHW("Vous avez efface ce champ...") D ^%VSQUEAK,PAD^%VZEATT H 2 D POCLEPA^%VVIDEO
 G LOOP
...
VERIF Q:'($D(^%SCRE(SCR,IC,6)))  S NVLEG=^%SCRE(SCR,IC,6) Q:NVLEG'>0
 ...
 S REFUSE=1 Q
FVER Q
REFULOP S ICC=IC D ^%VREFUSE G LOOP
...
EFF S NIC=IC-1 F UU=0:0 S NIC=$N(RESUL(NIC)) Q:NIC=-1  S DX=^%SCRE(SCR,NIC,2)+$L(^%SCRE(SCR,NIC,1)),DY=^%SCRE(SCR,NIC,3) X XY S CRIC=RESUL(NIC),RESUL(NIC)="",LIC=$L(CRIC) F VV=1:1:LIC W " "
 G LOOP
