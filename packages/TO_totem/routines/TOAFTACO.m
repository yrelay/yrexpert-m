...
;TOAFTACO^INT^1^59547,74871^0
TOAFTACO ;
 
 D CLEPAG^%VVIDEO S LILIM=20,DEV="ECR",DXG=0,DXD=39,DYH=0,DYB=4,GRAPH=1,MSG=$$^%QZCHW("Objets du SAVOIR-FAIRE") D TO4^%VZCD
 S PAGE=1,DX=0,DY=6 D CLEBAS^%VVIDEO G AFF1
...
AFF1 S DY=-1 F %QY=1:1 S DY=$N(^POSENR($I,PAGE,DY)) Q:DY=-1  D AFF2
 G SUIT
END D MAJ K PAG,LILIM,LOSTAR,LSTAR,%FIN,DEBP,INEXT,LI,I,PY,PX,NEXT,PAGE,U,CTRLE,CTRLA,CTRLF,CTRLL,CTRLZ,Y1,YG,NPAG,%QY,PAR,TYP Q
...
ECORD S DD=^POSFUL($I,PG),FF=$P(DD,"^",2),DD=$P(DD,"^",1)
 W !,$$^%QZCHW("Page "),$S(PG<10:" ",1:""),PG,?9,DD,!,?9,FF D:$Y'<21 ALOR
 G FIN
...
MAJ Q:'($D(^POSENR($I)))  Q:'($D(^VALPAR($I)))  D POCLEPA^%VVIDEO W $$^%QZCHW("Un instant...")
 S PG=-1 F I=1:1 S PG=$N(^VALPAR($I,PG)) Q:PG=-1  S LI=-1 F JJ=1:1 S LI=$N(^VALPAR($I,PG,LI)) Q:LI=-1  D DEL
 G FIN
DEL G:'($D(^POSENR($I,PG,LI))) FIN S LIB=$P(^POSENR($I,PG,LI),"^",1),OBJ=$P(^POSENR($I,PG,LI),"^",2)
 D ^TODELOBJ G FIN
