...
;TOGENPOP^INT^1^59547,74872^0
TOGENPOP ;
 
 S TIT=$$^%QZCHW("Gen. fiches para. associes aux oper.") D ^TOGETART
...
TRAIT0 S PH=-1 F %PP=1:1 S PH=$N(^[QUI]PHASE(ARTI,PH)) Q:PH=-1  S NUOR=-1 F %NN=1:1 S NUOR=$N(^[QUI]PHASE(ARTI,PH,NUOR)) Q:NUOR=-1  S OP=-1 F %OO=1:1 S OP=$N(^[QUI]PHASE(ARTI,PH,NUOR,OP)) Q:OP=-1  D TRAIT
 Q
TRAIT Q:'($D(^[QUI]PHASE(ARTI,PH,NUOR,OP)))  G:$D(^[QUI]SPROPARA(ARTI,PH,OP)) FINTR W "." S ^[QUI]SPROPARA(ARTI,PH,OP,1)="",^[QUI]FLSPROPA(ARTI,PH,OP,1)="",^[QUI]SPROPVAL(ARTI,PH,OP,1,1)="",^[QUI]FLSPROPV(ARTI,PH,OP,1,1)=""
FINTR Q
