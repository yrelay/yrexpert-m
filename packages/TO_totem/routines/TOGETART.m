...
;TOGETART^INT^1^59547,74872^0
TOGETART ;
 
 K ^TACOMPIL($I),^COMPILVU($I)
QUES S %NARTI=1 D CLEPAG^%VVIDEO,POCLEPA^%VVIDEO W $$^%QZCHW("Article suivi de ""*"" : Article plus ses composants,(RETURN)pour fin liste")
 S DXG=0,DXD=38,DYH=0,DYB=4,GRAPH=1,MSG=TIT,BIG=1 D TO4^%VZCD S DX=10,DY=6 X XY S PPX=DX,PPY=DY
QUES2 S DX=6 X XY S AX=$X,AY=$Y D ^%VLEC G:CTRLA=1 FIN G FIN:Y1="",NONUN:Y1="*" S STAR=$E(Y1,$L(Y1))="*" S:STAR Y1=$E(Y1,1,$L(Y1)-1) D ^%LXMOT(Y1,AX,AY,.%OK,.%F) W:STAR "*" G NONUN:'(%OK)
