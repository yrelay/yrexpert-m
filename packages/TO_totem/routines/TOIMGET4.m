...
;TOIMGET4^INT^1^59547,74872^0
TOIMGET2 ;
 
 S:'($D(@%TBEV@("%NUA"))) @%TBEV@("%NUA")="?" S:@%TBEV@("%NUA")="" @%TBEV@("%NUA")="?"
 S VU=$S(@%TBEV@("%NUA")="?":1,1:0),TOZEC=^TOZE($I,"C"),TOZEA=^TOZE($I,"A"),%LIEU=$$^%QZCHW("Implicites"),%TABRA="^[QUI]RAISON",%TABTV="^[QUI]TVPAR" D ^TO3WLIEU G:'($D(^[QUI]TRI(YA))) FIN
 S NOMTREE="^[QUI]TRI("""_YA_"""",%TREECOP="TRI("""_YA_"""" D ^%QCAGLC
 S NTABN="IMPLICI",NCLE=3,PNUM=2,F1=18,NFOR=1,NTRI="",PARA=$$^%QZCHW(" "),CONDIT=0 S:@%TBEV@("%NUA")'["?" NTRI=@%TBEV@("%NUA")-1
 I '($D(^[QUI]FULL(YA,"DATE"))) D ZD^%QMDATE4,^%QMDAK0 S ^[QUI]TVPAR(NUFA,TWREF,"DATE")=%DAT_$$^%QZCHW(" a ")_HEURE,^[QUI]RAISON(NUFA,TWREF,"DATE")="IMPLICITE MODIFICATION",^[QUI]FULL(YA,"DATE")=1
LOPAR S:'($D(VU)) VU=1 S NTRI=$O(TRI(YA,NTRI)) G:(NTRI'=@%TBEV@("%NUA"))&(VU=0) LOPAR G:%ABEND'=0 FIN G:NTRI="" FIN
 S VU=1,PRX=TRI(YA,NTRI)
 S SOURCE=$E(PRX,2,$L(PRX)-1),PARA=$P(PRX,",",2),PARAP=$P(PARA,"""",2) G:$D(^V($I,YA,PARAP)) LOPAR I (PARAP'["$ARG")&(PARAP'["%VAL") S ANC=PARAP D ^TOEVCOUR
...
RBLI I PARAP=$$^%QZCHW("ARRET"),VPA=1 D ABEND G LOPAR
