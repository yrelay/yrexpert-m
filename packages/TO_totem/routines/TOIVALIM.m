...
;TOIVALIM^INT^1^59547,74872^0
TOIVALIM ;
 S $ZT="" K TVL,%NBDEF
 K ^DOLDMD($I) S %RESULTAT=0,TVL("%NBDEF")=0,TVL("%NBSOL")=1,TVL($$^%QZCHW("TRACE"))=0 K ^GRSOL($I) S %RESULTAT=0
 S @("TVL(""NUGROU"")="_$P(EXPFONC,"/",2)),@("TVL(""SSGRO"")="_$P(EXPFONC,"/",3)),@("TVL(""TYP"")="_$P(EXPFONC,"/",4)),TVL("TYPN")=$P(EXPFONC,"/",5) S:TVL("TYPN")=0 TVL("TYPN")="SANS" S QUW=$S(TVL("TYP")=0:^TABIDENT($I,"%UCI"),1:QUI)
 I TVL("TYPN")="SANS" G TT
...
RET1 G:%ABEND=1 FIN S TVL("NORM")=@TVL("CTYPN") S:(TVL("NORM")="TOUTES")!((TVL("NORM")="TOUS")!(TVL("NORM")="SANS")) TVL("TYPN")="SANS" G:TVL("TYPN")="SANS" TT G T0
TT S TVL("NORM")=-1
...
LPR S TVL("PPGR")=$N(^GRIL($I,TVL("PPGR"))) G:TVL("PPGR")=-1 LPRS
 S TVL("VPAR")=$S($D(^V($I,YA,TVL("PPGR"))):^V($I,YA,TVL("PPGR")),1:"???") S:TVL("VPAR")["??" TVL("IND")=1 S ^VPAR($I,TVL("PPGR"))=TVL("VPAR"),^TBPARV($I,TVL("PPGR"))=TVL("VPAR"),TVL("%REFD")=TVL("%REFD")_","""_TVL("VPAR")_""""
 I TVL("VPAR")'["??" S TVL("VAL")=^VPAR($I,TVL("PPGR")),TVL("UNI")="E" D UNITE S ^TBPARV($I,TVL("PPGR"))=TVL("VAL"),^VPAR($I,TVL("PPGR"))=TVL("VAL")
 G LPR
LPRS I $D(^[QUW]GROUGRIL(NUGR)) S ANC=$$^%QZCHW("Abaque : ")_$P($P(^[QUW]GROUGRIL(NUGR),"|",2),"^",1) D ^TOEVCOUR
...
AFF S TVL("PPGR")=TVL("PRESU"),TVL("%VV")=@%TAB@(TVL("PRESU")),TVL("VAL")=$P(TVL("%VV"),"|",1),TVL("UNI")="S" D UNITE S TVL("%VAL")=TVL("VAL"),TVL("%PG")=$P(TVL("%VV"),"|",2)
 S ^V($I,YA,TVL("PRESU"))=TVL("%VAL"),^M($I,%MC,TVL("PRESU"))=TVL("%VAL") D GETDEP Q
AAFF 
 G END
FIN K TVL,%NBDEF,%POR,VPA,PARA,%INT,%MODA
 K %TAB,%TAD Q
END Q
GETDEP 
...
GETB K TVL("AOPARA") S:$D(PARA) TVL("AOPARA")=PARA S:'($D(%LIEU)) %LIEU="" S VPA=TVL("%VAL"),%MODA=$S(%LIEU[$$^%QZCHW("Mat"):"MAT",1:"ART") D ^TO3PASYS S:$D(TVL("AOPARA")) PARA=TVL("AOPARA") G GETC
