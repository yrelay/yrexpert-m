...
;TOIVBATC^INT^1^59547,74872^0
TOIVBATC ;
 
DEB 
 D WHOIS^TOWWHOIS
 K ^V($I),^PSRL($J),^BUT3($J),^POSICOMP($I),^PILETUD($I),^TPBLIMP($I),^PHASADM($I),^GLOETUD($I),^NBRGLEV($I),^TENTATIV($I),^AIGNOR($I)
 ...
 D ^%QMDAK0 K ^ETBATCH($I),^BATCHCOU($I) S OBT="METB"_$P(%DAT,"/",3)_$P(%DAT,"/",2)_$P(%DAT,"/",1),OR=^[QUI]CPTEUR("METB",%DAT)+1,OBT=OBT_$S($L(OR)<2:0,1:"")_OR,^BATCHCOU($I)=OBT,^BATCHCOU($I,"PGE")=1,^BATCHCOU($I,"LI")=8 K OBT,OR
...
QU D ^TOIVGBUT G:'($D(^BUT($J))) FIN
QUE S:$D(^ETBATCH($I)) ^[QUI]CPTEUR("METB",%DAT)=^[QUI]CPTEUR("METB",%DAT)+1 S BATCH=1,%NUBATCH=0
QU1 S %NUBATCH=$N(^ETBATCH($I,%NUBATCH)) G:%NUBATCH=-1 QU2 D KILL S %ART=$P(^ETBATCH($I,%NUBATCH),"?",1) D ^TOIVEVAL G QU1
QU2 D ZD^%QMDATE4,^%QMDAK0 S ^[QUI]RESMETB(^BATCHCOU($I))=^BATCHCOU($I,"DEBUT")_%DAT_"|"_HEURE
FIN Q
