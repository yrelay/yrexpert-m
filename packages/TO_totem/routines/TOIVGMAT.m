...
LOPAR 
 S GMT("PRX")="("""_YA_""","_$P(^[QUI]IREPFICH(YA,%REG),"^",2)_")",SOURCE=$E(GMT("PRX"),2,$L(GMT("PRX"))-1) D TESCOND S %ABEND=0 G END
TESCOND 
RETERR Q:%ABEND=1
 S RBIS=GMT("PRX")
 D ^TOTSDEFA($$^%QZCHW("MATIERE"))
 K RBIS
 G:'(TSDEF) FIN
 K TSDEF
 S $ZT=^TOZE($I,"C")
...
FIN Q
END K GMT,SOURCE,RBIS,TSDEF,%TBL,CONTAB,NTABN,NCLE,PNUM,NFOR,PARA
 
ENDF K GMT,GMT2,CONTAB,ZZ Q
YAERR S xxx=zzz Q
...
TRAIT S %ABEND=0 I '($D(^[QUI]MATSTOCK(FMS))) G TTT
 S GMT2($$^%QZCHW("MATAS"))=FMS,^V($I,YA,$$^%QZCHW("MATIERE"))=GMT2($$^%QZCHW("MATAS")),MSS=GMT2($$^%QZCHW("MATAS")),GMT2("SEP")=$S(GMT2($$^%QZCHW("MATAS"))[$$^%QZCHW(" "):$$^%QZCHW(" "),1:".")
 S FMS=$P(GMT2($$^%QZCHW("MATAS")),GMT2("SEP"),1),SFMAT=FMS_GMT2("SEP")_$P(GMT2($$^%QZCHW("MATAS")),GMT2("SEP"),2)
 D ^TO3GPMAT Q:%ABEND=1  S MATSEL(TWREF,GMT2($$^%QZCHW("MATAS")),1)=1_"^"_GMT2($$^%QZCHW("MATAS"))_"^"_GMT2($$^%QZCHW("MATAS")) D AFFMAT Q
TTT K GMTS("OOSTOP") S:$D(STOP) GMTS("OOSTOP")=STOP S NOMTAB="^[QUI]SELMAT("""_YA_""","""_FMS_"""",ADR="^TOIVGMAT" D ^%VTENR S:$D(GMTS("OOSTOP")) STOP=GMTS("OOSTOP") Q
