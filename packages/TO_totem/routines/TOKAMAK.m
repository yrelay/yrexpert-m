...
;TOKAMAK^INT^1^59547,74872^0
TOKAMAK ;;06:10 PM  7 Oct 1989
 ;;
 
 ;; on a conserve la plupart des options de TKPOP
 G BEG
 ;;
...
BEG D:$D(TK)=0 INIT^TKDEV ;; initialisation eventuelle de TK
 I $P(Y,"\",5)'="",$D(^TKPAINT($P(Y,"\",5),0,^TK($I),1)) F TI=1:1 Q:$D(^(TI))=0  W ^(TI) ;; impression d'une image s'il y en a une dans les options d'appel
 S TC=$P(Y,"\",3),TP=$P(Y,"\",2) ;; tc est le nombre de colonnes
 D filly
Y I $D(Y(0)) S TA=Y(0)
...
LIST I $L($P(Y,"\",4)) S %C=(((TW-$L($P(Y,"\",4)))\2)+%C)+1,%R=TT W @(TKP),$P(Y,"\",4)
 S TM=0,TE="TA="_TKP,TH="" F TJ=1:1:TC S %C=TC(TJ),%R=TT F TM=TM+1:1:TM+TD Q:$D(Y(TM))=0  S %R=%R+1,@(TE),TX=" "_Y(TM) S:TX'["*" TB(TM)=TA_TX_" ",TH=TH_$E(TX,1,5)_"\" S:TX["*" TX=$P(TX,"*"),TH=TH_"\" W TA,TX S ^pospop($J,TM)=%R_","_%C_","_TX
...
SEL0 F X=X:0:TI W TR,TB(X) R TE#1 X TM G:ZF=$C(27) END G:ZF=$C(13) SEL1 W TRO,TB(X) G:TE?1NA SEL2 S @($S(ZF=TK("UK"):"X=X-1",ZF=TK("RK"):"X=X+TD",ZF=TK("LK"):"X=X-TD",1:"X=+$O(TB(X))")) D:$D(TB(X))=0 TT
 S X=$O(TB(0)) G SEL0
...
filly ;; transform la matrice m en vecteur y
 S m=$P(Y,"\",6),nbli=$P(m,",",2),nbcol=$P(m,",",3),m=$P(m,",",1)
 K m1 S m1=$P(Y,"\",7) F %i=1:1:m1 S m1($P(m1,",",%i))=1
 F %i=1:1:nbli F %j=1:1:nbcol S Y(((%j-1)*nbli)+%i)=@m@(%i,%j)_$S($D(m1(%j)):"",1:"*")
 K m1,m
 Q
read ;; lecture et modif de la matrice
 S %C=$P(^pospop($J,X),",",2),%R=$P(^pospop($J,X),",",1) K PROMPT S X=$P(^pospop($J,X),",",3)
 S LNG=60 X ^TKREAD S:RX=0 poste=X
 S:RX=6 fin=1
 Q
load ;;	
