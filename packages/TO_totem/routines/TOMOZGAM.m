...
;TOMOZGAM^INT^1^59547,74873^0
TOMOZGAM(NUFA,TWREF,LISTE) 
...
LPH S NUPH=$N(^[QUI]PHAS(NUFA,TWREF,NUPH)) G:NUPH=-1 FPH K CH S OPER=NUPH,NUOP=^[QUI]PHAS(NUFA,TWREF,NUPH),ACT=$P(NUOP,"/",1),MAC=$S($D(^[QUI]SECT(NUFA,TWREF,NUOP)):^[QUI]SECT(NUFA,TWREF,NUOP),1:"???"),MAC=$$S^%QAX(MAC) K COM S NCOM=1
 I $D(^[QUI]CONS(NUFA,TWREF,NUOP)) S COM(NCOM)=$$^%QZCHW("Consignes")_" : "_^[QUI]CONS(NUFA,TWREF,NUOP),NCOM=NCOM+1 D ATTC(NUOP_".001",^[QUI]CONS(NUFA,TWREF,NUOP))
 K DIM S NDIM=-1 F %U=1:1 S NDIM=$N(^[QUI]DIM(NUFA,TWREF,NUOP,NDIM)) Q:NDIM=-1  S DIM=^[QUI]DIM(NUFA,TWREF,NUOP,NDIM),COM(NCOM)=$P(DIM,"|",1)_" = "_$P(DIM,"|",2),^DIM($J,NUOP_".001",%U)=COM(NCOM),NCOM=NCOM+1
 S DUR=$S(($D(^[QUI]DUREE(NUFA,TWREF,NUOP))#10)=1:$P(^[QUI]DUREE(NUFA,TWREF,NUOP),"/",1),1:"???^???"),TPR=$P(DUR,"^",1),TPR=$S(TPR["?":TPR,1:$J(TPR,0,2)),TPF=$P(DUR,"^",2),TPF=$S(TPF["?":TPF,1:$J(TPF*QT,0,2))
