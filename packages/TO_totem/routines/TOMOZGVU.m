...
;TOMOZGVU^INT^1^59547,74873^0
TOMOZGVU ;
 D CLEPAG^%VVIDEO S DXG=1,DYH=0,DXD=38,DYB=4,MSG=ART_" - "_PCH_"*",GRAPH=1 D TO4^%VZCD
...
T1 S LIG=^TACOMPIL($J,PG,LI) K T F %V=1:1:4 S T(%V)=$P(LIG,"^",%V)
 S T(2)=T(3),T(3)=T(4)
...
T0 S CH="" F %W=0:1:2 S CH=CH_$P(FIC,"^",%W+%V)_"^"
 ...
 Q
TAB K ^TACOMPIL($J) S:PCH="" PCH="TOT" S PG=1,NU=0,LI=11,CC=$E(PCH,1,$L(PCH)-1)_$C($A($E(PCH,$L(PCH)))-1)_"z"
LPCH S CC=$N(@TB@(CC)) G:CC=-1 FPCH G:$E(CC,1,$L(PCH))'=PCH FPCH S TW=-1 D LTW G LPCH
LTW S TW=$N(@TB@(CC,TW)) Q:TW=-1  G:'($D(@("^[QUI]TREEWORK(CC,"_TW_")"))) LTW S ARTI=$P(@("^[QUI]TREEWORK(CC,"_TW_")"),"^",2) G:'($D(^[QUI]ANTETUDE(ARTI,CC,TW))) LTW S ET=^[QUI]ANTETUDE(ARTI,CC,TW),HE=$P(ET,"^",4),ET=$P(ET,"^",3)
 S NU=NU+1,^TACOMPIL($J,PG,LI)=CC_"^"_TW_"^"_ET_"^"_HE_"^"_ARTI,LI=LI+1 I LI>21 S PG=PG+1,LI=11
 G LTW
FPCH S NBP=$S($D(^TACOMPIL($J,PG)):PG,1:PG-1) K CC,TW,ET,LI,PG,ARTI,TB Q
COM ;;Etudes,12/Dates,46/Heures,67
