...
;TOP0CHNU^INT^1^59547,74873^0
TOP0CHNU ;
 
 K ^EDICLI($J),^POSENR($I) S NBCLCH=0,PAGE=0,LG=25,ARC=-1 F AA=1:1 S ARC=$N(^GLOCLI($I,ARC)) Q:ARC=-1  D TRAIT
 I NBCLCH=1 K ^POSENR($I)
FIN I $D(^POSENR($I)) D ^TOP0CHET
...
STR1 S NF=AFUN F %FN=1:1 Q:NF=-1  D:$D(^[QUI]TREEWORK(NF)) TR3 S NF=$N(^[QUI]TREEWORK(NF)) Q:NF'[AFUN
 Q
TR2 S FANU=AFUN,TWREF=-1 F TT=1:1 S TWREF=$N(^[QUI]TREEWORK(FANU,TWREF)) Q:TWREF=-1  S TRWK=^[QUI]TREEWORK(FANU,TWREF),ARC=$P(TRWK,"^",10) I (ARC'="")&(ARC'=" ") S ^EDICLI($J,ARC,FANU,TWREF)=1,NF=FANU D TR3 K NF
 Q
TR3 S LG=LG+1 I LG>21 S LG=8,PAGE=PAGE+1
