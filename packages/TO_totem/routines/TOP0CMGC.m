...
;TOP0CMGC^INT^1^59547,74873^0
TOP0CMGC ;
 
LOOP K VALCLE S RECOM=0,IFLAG=0,ADR="LOOP1^TOP0CMGC",VALCLE(1)=NR,VALCLE(2)=RESUL(1)
 D ^%VKMULG G FIN:STOP=1,CHAN2
LOOP1 K RESUL S RESUL(1)=B2 F UU=1:1:^%SCRE(SCR) S RESUL(UU+1)=$S($D(%RS(UU)):%RS(UU),1:"")
...
D G LOOP11 S (%PCOUR,%NBPVU)=1 D ACCOR G:'(ACCOR) LOOP11 S OAJCMS=$S($D(%AJCMS):%AJCMS,1:""),%AJCMS="Duplication",ABENDSCR=0
 D POCLEPA^%VVIDEO W *7,"N'oubliez pas de changer les champs cles" H 2 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^%VKMODIS S %AJCMS=OAJCMS G LOOP11:ABENDSCR=1 D ^TOP0STOC S ^PRESCR($I,SCR,%NBPVU)=NOMTAB_"("_$P(NOM,"(",2,599),%PCOUR=%NBPVU,%NBPVU=%NBPVU+1 D NEWOLD G LOOP11
