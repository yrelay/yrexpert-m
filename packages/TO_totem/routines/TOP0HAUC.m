...
;TOP0HAUC^INT^1^59547,74873^0
TOP0HAUC ;
 
 S %PGM="TOP0HAUD" D PREPAR,^TOP0AFF,^%QUAGLO O IMPR
FIN Q
PREPAR K ^PARPARTI(DOLARI) S ^PARPARTI(DOLARI,"TYPE.SCIAGE")="",SCIPLA=$S($D(^[QUI]PARMAT(NUFA,TWREF,"SCIAGE.PLATEAU")):^[QUI]PARMAT(NUFA,TWREF,"SCIAGE.PLATEAU"),1:"") I SCIPLA="OUI" S ^PARPARTI(DOLARI,"TYPE.SCIAGE")="SCIEES"
 S SCIPI=$S($D(^[QUI]PARMAT(NUFA,TWREF,"SCIAGE.PIED")):^[QUI]PARMAT(NUFA,TWREF,"SCIAGE.PIED"),1:"") I SCIPI="OUI" S ^PARPARTI(DOLARI,"TYPE.SCIAGE")="SCIAGE PIED"
 S MAT=$S($D(^[QUI]PARMAT(NUFA,TWREF,"MATIERE")):^[QUI]PARMAT(NUFA,TWREF,"MATIERE"),1:""),^PARPARTI(DOLARI,"MATIERE")=$P(MAT,".",2)
 S PHE=$N(^[QUI]QUERY3("P0",ARC,1,"PHE",-1)),^PARPARTI(DOLARI,"PHE")=$P(PHE," ",1)_$P(PHE," ",2)
 S DPTO=$N(^[QUI]QUERY3("P0",ARC,1,"DATE.AU.PLUS.TOT",-1)),^PARPARTI(DOLARI,"DATE.AU.PLUS.TOT")=$E(DPTO,3,4)_$E(DPTO,1,2)
 S DPTA=$N(^[QUI]QUERY3("P0",ARC,1,"DATE.AU.PLUS.TARD",-1)),^PARPARTI(DOLARI,"DATE.AU.PLUS.TARD")=$E(DPTA,3,4)_$E(DPTA,1,2)
 S ^PARPARTI(DOLARI,"NUMERO.COMMANDE")=$E(ARC,1,$L(ARC)-2),^PARPARTI(DOLARI,"POSTE")=$E(ARC,$L(ARC)-1,$L(ARC))
 F J=0:1:4 S L=$P($T(PARQU+J),";;",2) F I=1:1 S P=$P(L,"~",I) Q:P=""  Q:'($D(^[QUI]QUERY3("P0",ARC,1,P)))  S ^PARPARTI(DOLARI,P)=$N(^[QUI]QUERY3("P0",ARC,1,P,-1))
 Q
