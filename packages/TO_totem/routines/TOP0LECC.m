...
ERREUR I '(DTM),$ZE["TAP" U 47 W *-2 G:NUCLI=0 TR K ^[QUI]CLICHE(DATE,HOUR,NUCLI),^[QUI]ERLECBD(DATE,HOUR,NUCLI) S ^[QUI]ERLECBD(DATE,HOUR,"STOP")=NUCLI-1 G FIN
 I DTM,$$^%SYSBUG($ZERR)["TAP" U 47 W *-2 G:NUCLI=0 TR K ^[QUI]CLICHE(DATE,HOUR,NUCLI),^[QUI]ERLECBD(DATE,HOUR,NUCLI) S ^[QUI]ERLECBD(DATE,HOUR,"STOP")=NUCLI-1 G FIN
...
LOOP S DEBUT=0,CONCAT=0 F LOP=1:1 U 47 R X S DZA=$ZA U 0 D ERROR Q:FIN=1  S PARA=$E(X,1,20) D ^%QZCHBT U 0 D STOENR
 Q
STOENR I PARA="1.I" S DEBUT=1,NUCLI=NUCLI+1,LG=-1 W "." S OOX=$X,OOY=$Y D POCLEPA^%VVIDEO W "Cliche No : ",NUCLI S DX=OOX,DY=OOY X XY
