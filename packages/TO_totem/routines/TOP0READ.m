...
LOOP S %ENR=0,DEBUT=0,CONCAT=0 F LOP=1:1 U 47 R X S DZA=$ZA,XR=$TR(X," ","|"),%ENR=%ENR+1,^ENREG(%ENR)=XR U 0 W !,XR D ERROR Q:FIN=1  S PARA=$E(X,1,20) D ^%QZCHBT U 0 D STOENR
 Q
STOENR I PARA="1.I" S ^NEWENREG(%ENR)=1,%PENR=1,DEBUT=1,NUCLI=NUCLI+1,LG=-1 W "." S OOX=$X,OOY=$Y D POCLEPA^%VVIDEO W "Cliche No : ",NUCLI S DX=OOX,DY=OOY X XY
...
PB I PB&(NUCLI'=0) S ^[QUI]ERLECBD(DATE,HOUR,NUCLI,LG)=1
 Q
ARC D @WHOIS S %PENR=0,%LIB=ENR_$E(%NCH,1,53),%ARC=$$GETLBLAN^%VTLBLAN($E(%LIB,PED,RRA)) S:%ARC="" %ARC=1
