...
1 G END
83 G:$D(^EDICLI($J,ARC,NUFA)) DEB1 S TWREF=-1 F TW=1:1 S TWREF=$N(^[QUI]TREEWORK(NUFA,TWREF)) Q:TWREF=-1  S ^EDICLI($J,ARC,NUFA,TWREF)=1
 D BLD^%VVIDEO,BLK^%VVIDEO S DX=0,DY=PY X XY W "S" D AFFLG,NORM^%VVIDEO
 G DEB1
7 I $D(^EDICLI($J,ARC,NUFA)) K ^EDICLI($J,ARC,NUFA) S DX=0,DY=PY X XY W "  " D AFFLG
 G DEB1
