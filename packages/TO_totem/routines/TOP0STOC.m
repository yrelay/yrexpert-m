...
;TOP0STOC^INT^1^59547,74874^0
TOP0STOC ;
 
 Q:('($D(NOMTAB)))!('($D(SCR)))
 S %EXIST=0,INL=1,LLS=0 F I=1:1:^%SCRE(SCR) S LLS=LLS+$L(RESUL(I))
...
REFUL D ^%VSQUEAK,POCLEPA^%VVIDEO W "Informations trop abondantes : veuillez abreger les champs " H 2
 ...
 H 2 D ^%VAFIGRI,^%VAFISCR S IC=1,ABENDSCR=0 D ^%VKMODIS G TOP0STOC:ABENDSCR=0 Q
REFUS D ^%VSQUEAK,POCLEPA^%VVIDEO W "Il faut obligatoirement une information dans  ",^%SCRE(SCR,IC,1) H 2 D ^%VAFIGRI,^%VAFISCR S ABENDSCR=0 D LOOP^%VKMODIS G TOP0STOC:ABENDSCR=0 Q
LONG S IN=""""_NR_"""",RS="" D BEG^%VKLONS Q
