...
DEB1 S DY=PY+1 S:DY>21 (DY,PY)=21 S DX=0 X XY D ^%VZECUR G:X1=1 1 S PY=$Y
 G:'($D(^POSENR($I,PAGE,PY))) DEB1 G:(((((X1'=1)&(X1'=83))&(X1'=86))&(X1'=80))&(X1'=7))&(X1'=6) DEB1
 G @X1
FIN Q
...
86 S ^PAGE($J)=PAGE D ^TOP0VIFL S (OPAGE,PAGE)=^PAGE($J),%QUIT=1,IOM=80 D CADRE^TOP0LICL,BEG1^TOP0LICL K ^PAGE($J) G AFPAG
80 S ^PAGE($J)=PAGE D ^TOP0VIZS S (OPAGE,PAGE)=^PAGE($J),%QUIT=1,IOM=80 D CADRE^TOP0LICL,BEG1^TOP0LICL K ^PAGE($J) G AFPAG
AFFI S COL=1 F XX=3,15,40,55,67 W ?XX,$P(^POSENR($I,PAGE,PY),"^",COL) S COL=COL+1
 Q
AFPAG S PAG="Page "_PAGE_"/"_NBPAG,DX=40-($L(PAG)\2),DY=5 X XY W PAG G TOP0TRCL
