...
;TOPBCHNU^INT^1^59547,74874^0
TOP0CHET ;
 
 K ^SELEC($I) S NBPAG=$S($D(^POSENR($I,PAGE)):PAGE,1:PAGE-1),PAGE=1,DX=0,DY=6 D CLEBAS^%VVIDEO,ENTET1 G AFF1
SUIT D POCLEPA^%VVIDEO W $$L20^%QSLIB2("aux objets,?:Detail,(T)ous") S PAG="Page "_PAGE_"/"_NBPAG
...
ON I $D(^SELEC($I,PAGE,DY)) D BLD^%VVIDEO,BLK^%VVIDEO S DX=0 X XY W "S"
 G FIN
FIN Q
ENTET1 S DX=0,DY=6 X XY W ?4,"ARC",?31,"No d'etude",?60,"Date"
 Q
TOUS F PAGE=1:1:NBPAG F LG=8:1:21 Q:'($D(^POSENR($I,PAGE,LG)))  S DATA=^POSENR($I,PAGE,LG),ARC=$P(DATA,"^",1),NUFA=$P(DATA,"^",2),TWREF=-1 F TW=1:1 S TWREF=$N(^[QUI]TREEWORK(NUFA,TWREF)) Q:TWREF=-1  S ^SELEC($I,PAGE,LG)=NUFA_"^"_TWREF_"^"_ARC
 Q
