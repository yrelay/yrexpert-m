...
;TOREAUS1^INT^1^59547,74874^0
TOREAUS1 ;
 
 S %ABAND=0,DX=0,DY=7 X XY D ^%AB3STAR K ^POSENR($I),^VBIS($I),VALPAR($I),^PARVAL($I),^DON($I),^POSFUL($I) S PAGE=1
...
EXP S NEXT=$N(^[QUI]EXPLICI(USI,INEXT,-1))
 S ^POSENR($I,PAGE,LI)=NEXT_"^(E)",^POSFUL($I,PAGE,LI)=NEXT,^PARVAL($I,NEXT)="",^DON($I,NEXT)=I,^VALPAR($I,NEXT)="",LI=LI+2 I LI>21 S LI=8,PAGE=PAGE+1
FIN Q
SUIT S %FIN=1 D POCLEPA^%VVIDEO W $$L20^%QSLIB2("aux parametres,CTRLF:Fin") S PAG="Page "_PAGE,DX=40-($L(PAG)\2),DY=6 X XY W "Page ",PAGE S DX=$X-1,DY=$Y D ^%VLEC G 1:CTRLA,6:CTRLF,65:CTRLE,61:(Y1="+")!(Y1="="),45:Y1="-"
SINON S NPAG=Y1 D AFF G SUIT
1 S %ABAND=1,STOP=1,%ABEND=1 G END
6 S STOP=1,%ABAND=0 G END
65 D ^TOREAUS2 G SUIT
43 G 61
...
AFF Q:$A(NPAG)<32  I '($D(^POSENR($I,NPAG))) D ^%VSQUEAK G SUIT
 ...
 Q
ECR S DY=DDY,DX=0,PAR=$P(^POSENR($I,PAGE,DY),"^",1),TYP=$P(^POSENR($I,PAGE,DY),"^",2) X XY W $E(PAR,1,37),?40," = ",^VALPAR($I,PAR) Q
