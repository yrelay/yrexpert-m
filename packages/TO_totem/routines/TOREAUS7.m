...
;TOREAUS7^INT^1^59547,74874^0
TOREAUS7 ;
 
 D L7^%QSLIB S TYMOD="Saisie" D ^TO3TYMOD S (PY,DY)=DEBP-1,DX=43 X XY
DEB S DY=PY+2,DX=43 X XY S MPASX=0,MPASY=2,NXI=42,NXS=44,NYI=7,NYS=21 X XY D ^TOREAUS8 G:(X1=1)!(X1=6) END G DEB
FIN Q
...
EXP S NEXT=$N(^[QUI]EXPLICI(USI,INEXT,-1))
 S ^POSENR($I,PAGE,LI)=NEXT,^POSFUL($I,PAGE,LI)=NEXT,^DON($I,NEXT)=I S:'($D(^VALPAR($I,USI,NEXT))) ^VALPAR($I,USI,NEXT)="" S:'($D(^PARVAL($I,USI,NEXT))) ^PARVAL($I,USI,NEXT)="" S LI=LI+2 I LI>(LILIM+1) S PAGE=PAGE+1,LI=8
 Q
