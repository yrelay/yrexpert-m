...
;TOREAUSI^INT^1^59547,74874^0
TOREAUSI ;
 
 K ^REFPAR($I),^USIVU($I),^BLOCSUP($I) S YB=YA,NUMUS=-1,NBS=0,%MODNU=0
...
SUIT S %OYA=YA,YA=USI D ^TO4GEXPL S YA=%OYA D ^TOREAUS1
...
S11 S %EXPERT=YB,NUM=-1,IDENT=USI_":"
 F UU=0:0 S NUM=$N(^TO4GEXPL($I,NUM)) Q:NUM=-1  S PARA=^PARVAL($I,^TO4GEXPL($I,NUM)),IDENT=IDENT_PARA_","
...
NUO D BLD^%VVIDEO S DX=0,DY=18 X XY W "No d'ordre dans la gamme : " S DX=$X D NORM^%VVIDEO,^%VLEC G:CTRLA!CTRLF FIN G:Y1="" NUO S NUORD=Y1
 
 S NBS=NBS+1,^BLOCSUP($I,YA,NBS)=IDENT_"^"_OPER_"^"_NUORD
FIN Q
TNBUO I '($D(ONUORD)) S ONUORD="" Q
 G:$D(%INCR)&(%MODNU=0) FTNUO
 S %MODNU=0,PED=$L(ONUORD),LUM=0 F %PP=PED:-1:1 S NBRE=$E(ONUORD,%PP) Q:NBRE'=0  S LUM=LUM+10
 S %INCR=$S(LUM=0:1,1:LUM)
FTNUO S ONUORD=ONURD+%INCR Q
