...
;TORESOEV^INT^1^59547,74875^0
TORESOEV ;
 
 S %DEC=$S($D(^TABIDENT(WHOIS,"DECIMALES")):^TABIDENT(WHOIS,"DECIMALES"),1:2),CTRLA=0,CTRLF=0
 D:DEV="ECR" CLEPAG^%VVIDEO W:DEV'="ECR" # D ENTET,EDIT Q:(CTRLF=1)!(CTRLA=1)
 W !,LSTAR W:DEV="IMPR" !,!,! I DEV="ECR" S DX=1,DY=24 X XY W $$^%QZCHW("entrer RETURN pour continuer.") R FR
 Q
EDIT S NT=-1,LI=0,STP=0
 S %TAB="^[QUI]ORDEVAL(NUFA,TWREF,""ART"")",%1=-1 F %2=1:1 S %1=$N(@%TAB@(%1)) Q:%1=-1  D AF Q:STP=1  S %11=-1 F %21=1:1 S %11=$N(@%TAB@(%1,%11)) Q:%11=-1  D AF2 Q:STP=1
 Q
...
AF2 S %13=$P(@%TAB@(%1,%11),"^",1) I (%13'["$ARG")&((%13'["%ARG")&(%13'["%VAL")) S RESULT=@%TAB@(%1,%11),XXD=6 D AFF
 Q
AFF S NT=$P(RESULT,"^",1),VL=$P(RESULT,"^",3),TYPAR=$$^%QZCHW("(I)") I $D(^[QUI]EXPL(YA,NT)),((^[QUI]EXPL(YA,NT)'="%%")&(^[QUI]EXPL(YA,NT)'="ABAC"))&(^[QUI]EXPL(YA,NT)'="I%%") S TYPAR=$$^%QZCHW("(E)")
 W !,"|",?XXD,NT,?47," = " D V W VL,?67,TYPAR D U W ?79,"|" I ((LI#3)=0)&(DEV="IMPR") H 2
 S LI=LI+1 I LI>LILIM D SUIT
 Q
V S PCH=VL D TO^%QZNBN1 I ISNUM=1 S VL=$J(VL,15,%DEC) Q
...
ENTET H:DEV'="ECR" 5 S %PPAGE=%PPAGE+1 D ^TOHAUPAG(IMPR,%DDATE,%HHEUR,%TTITR,%PPAGE)
 W !,!,LSTAR,!,"|",?10,NUC,?20,$$^%QZCHW("Parametres par ordre d'evaluation de : "),?79,"|",!,"|",?79,"|",!,"|",?10,TOID,?79,"|",!,LSTAR,!,"|",?79,"|"
 S LI=10 Q
