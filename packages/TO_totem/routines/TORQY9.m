...
TOT(GAM,REP,REPF,REPI,REPFI) 
 N TB,TBM,NPH,COM,LC,IDC,NLI,NM,TYP
GOG K ^OPVU($J)
 S TB="^U($J,0)",TBM="^[QUI]MOZART(GAM,0)",NPH="",COM=0,LC=""
 D INIT,INIS K @(TBM)
 K SOP S SOP=0,IDC="",NLI="",NM=0
LOP S NLI=$O(@TB@(NLI)) G:NLI="" FIN S LC=@TB@(NLI),TYP=$P(LC,"^",1)+0,LC=$P(LC,"^",2,999) S:TYP>3 TYP=3 G @TYP
0 G 3
1 K VAL,ATT I '($D(^TOR3($J,NLI))) D POP((TYP*4)_"^"_LC) G LOP
 D C(1) S SOP=5E-1,IDC="",NPH=ATT("NUMERO.OPERATION",1) D POP(TYP_"^"_NPH) G LOP
2 I ^TOR2($J,NLI)="" D POP((TYP*4)_"^"_LC) G LOP
 ...
 D PA^%QSGESTI(REPI,GAM,REPF,IDC,OP),PA^%QSGESTI(REPFI,GAM_","_IDC,REP,GAM,1),PA^%QSGESTI(REPFI,GAM_","_IDC,"NOM",IDC,1)
 S ATT="" F %U=1:1 S ATT=$O(ATT(ATT)) Q:ATT=""  S NOR="" F %U=1:1 S NOR=$O(ATT(ATT,NOR)) Q:NOR=""  D PA^%QSGESTI(REPFI,GAM_","_IDC,ATT,ATT(ATT,NOR),NOR)
 G LOP
3 G 31:IDC'="" D POP("3,"_NPH_","_SOP_"^"_LC) G LOP
31 I $E(LC)="*" D POP("3,"_NPH_","_SOP_"^"_LC) S SOP=0,NPH="",IDC="" K ATT G LOP
 I $E(LC)=";" D POP("3,"_NPH_","_SOP_"^"_LC) G LOP
 I '($D(NOR(IDC))) K NOR S NOR(IDC)=0
 S NOR=NOR(IDC)+1,NOR(IDC)=NOR D PA^%QSGESTI(REPFI,GAM_","_IDC,"COMMENTAIRE",$$GETLBLAN^%VTLBLAN(LC),NOR) G LOP
 
 
POP(CL) 
 S NM=NM+1,@TBM@(NM)=CL,@TBM@(0)=NM Q
C(I) 
...
INIT 
 S %TS=$S($D(^XMOZ("GRILLE",WHOIS)):WHOIS,1:"STAND") K MOZ D ADR^%QCAGLC("^XMOZ(""GRILLE"","""_%TS_"""","MOZ")
