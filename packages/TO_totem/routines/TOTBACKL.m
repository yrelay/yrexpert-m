...
;TOTBACKL^INT^1^59547,74875^0
TOTBACKL ;;12:23 PM  13 Apr 1989
 
 
 
 
BACK K ^L($J) S ^L($J,1)=%ART,^L($J,1,%ART)=%DOM,^L($J,1,%ART,%DOM)=%IMPL,(^L($J,$$^%QZCHW("HAUT")),^L($J,$$^%QZCHW("BAS")))=1 K ^TE($J),^TS($J),^TC($J),^TR($J),^TT($J)
 F %P=1:1 S P=^L($J,$$^%QZCHW("BAS")) Q:^L($J,$$^%QZCHW("HAUT"))<P  S A=^L($J,P),D=^L($J,P,A),I=^L($J,P,A,D),^L($J,$$^%QZCHW("BAS"))=P+1 D T1
 D MODIF S %TE="^TE($J)",%TS="^TS($J)",%TC="^TC($J)",%TR="^TR($J)" D ^TO4TRITO
FIN K A,A2,%A2,D,D2,%D2,DEFA,DEFI,F,%F,I,%I,LF,^L($J),P,P2,^TC($J),^TE($J),^TS($J),^TT($J),X1,X2 Q
...
T3 I '($D(^L($J,"TRAI",A2,D2,I2))) S P=^L($J,$$^%QZCHW("HAUT"))+1,^L($J,P)=A2,^L($J,P,A2)=D2,^L($J,P,A2,D2)=I2,^L($J,$$^%QZCHW("HAUT"))=P,^L($J,"TRAI",A2,D2,I2)=""
 S ^TT($J,A,D,I,F,A2,D2,I2)="" Q
 
SANPAR S LF(F,"NEANT")="",^TT($J,A,D,I,F,"NEANT","NEANT","NEANT")="" I $D(^[QUI]IREPFICH(A,F)),$P(^[QUI]IREPFICH(A,F),"^",3) S LF($$^%QZCHW("SINON"))=F Q
 
SINON Q:'($D(LF($$^%QZCHW("SINON"))))  S SF=LF($$^%QZCHW("SINON")),NF=-1 F %NF=0:0 S NF=$N(LF(NF)) Q:(NF=-1)!(NF=SF)  S X2=A_"^"_NF,X1=A_"^"_SF,^TE($J,X2,X1)=1,^TS($J,X1,X2)=1,^TC($J,X1)=$S($D(^TC($J,X1)):^TC($J,X1)+1,1:1) Q
 
MODIF S I="" F %I=0:0 S I=$Q(@("^TT("_$J_I_")")) Q:I'[("("_$J)  S I=$E(I,$F(I,"(",0),$L(I)-1),I=$E(I,$F(I,",",0)-1,$L(I)) D T
 S I="" F %I=0:0 S I=$Q(@("^TT("_$J_I_")")) Q:I'[("("_$J)  S I=$E(I,$F(I,"(",0),$L(I)-1),I=$E(I,$F(I,",",0)-1,$L(I)) X "S X1="_$P(I,",",2)_",X2="_$P(I,",",5) I '($D(^TC($J,X1_"^"_X2))) S ^TC($J,X1_"^"_X2)=0
 Q
T Q:I["""NEANT"",""NEANT"",""NEANT"""  S A=$P(I,",",2),F=$P(I,",",5),A2=$P(I,",",6),D2=$P(I,",",7),P2=$P(I,",",8) X "S AA="_A_",FA="_F_",A2A="_A2 S X1=AA_"^"_FA
 S F2=-1 F %F=0:0 S F2=$N(@("^TT("_$J_","_A2_","_D2_","_P2_","_F2_")")) Q:F2=-1  X "S F2A="_F2 S X2=A2A_"^"_F2A S:'($D(^TE($J,X2,X1))) ^TE($J,X2,X1)=1,^TS($J,X1,X2)=1,^TC($J,X1)=$S($D(^TC($J,X1)):^TC($J,X1)+1,1:1)
 Q
