...
;TOWCORLB^INT^1^59547,74876^0
TOWCORLB ;
 
LOOP K VALCLE S IFLAG=0,ADR="LOOP1^TOWCORLA",VALCLE(1)=LIEN
 S VALCLE(2)=RESUL(1)
 D ^%VKMULG G FIN:STOP=1,CHAN2
LOOP1 S RESUL(1)=B2 F UU=2:1 Q:'($D(%RS(UU-1)))  S RESUL(UU)=%RS(UU-1)
 S DX=0,DY=10 D CLEBAS^%VVIDEO,LOOP^%VAFIGRI,^%VAFISCR
LOOP11 D POCLEPA^%VVIDEO W $$^%QZCHW("(C)onsulter, (M)odifier, (S)upprimer, (A)jouter ")
LOOP2 R *X1 D TOUCHE^%INCCLAV(X1,.X1) G FIN:X1=6,FIN:X1<2 S:X1>91 X1=X1-32
 S ISN=$S(X1=77:$$^%QZCHW("M"),X1=83:$$^%QZCHW("S"),X1=67:$$^%QZCHW("C"),X1=65:"A",1:"REFUS") D POCLEPA^%VVIDEO G @ISN
...
M D ACCOR G:'(ACCOR) LOOP11 S ABENDSCR=0 D ^%VKMODIS G LOOP1:ABENDSCR=1
 K @(OLDNOM),@(OLDFULL) D ^TO31STOC,NEWOLD G LOOP11
