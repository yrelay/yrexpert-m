...
B I ^[QU]SCRC(SCR)=0 D POCLEPA^%VVIDEO,^%VSQUEAK W $$^%QZCHW("DESOLE !! NON ENCORE DISPONIBLE... ") H 3 D POCLEPA^%VVIDEO G FIN
 ...
 S:DY>DY1 DY=DY1
 D CLEBAS^%VVIDEO
SUITE K ^IGCURGRI($I,SCR),^GCURGRIL($I,SCR) F ICC=1:1:^[QU]SCRC(SCR) S XX=^[QU]SCRC(SCR,ICC,2),YY=^[QU]SCRC(SCR,ICC,3),^GCURGRIL($I,SCR,YY,XX)=ICC,^IGCURGRI($I,SCR,-(YY),XX)=1
 F ICC=1:1:^[QU]SCRC(SCR) S RESUL(ICC)="",DEJALU(ICC)=""
...
LOP3 S YG=RESUL(ICC),C=^[QU]SCRC(SCR,ICC,1),DX=^[QU]SCRC(SCR,ICC,2),DY=^[QU]SCRC(SCR,ICC,3) X XY
 ...
 D VERIF G REFULOP:REFUSE=1 I $D(CMS),CMS'=1,$D(^[QU]SCRC(SCR,ICC,4)),^[QU]SCRC(SCR,ICC,4)'="" F %UU=1:1 S UCONT=$P(^[QU]SCRC(SCR,ICC,4),",",%UU) Q:UCONT=""  S IC=ICC D @UCONT G REFULOP:REFUSE=1
 Q
