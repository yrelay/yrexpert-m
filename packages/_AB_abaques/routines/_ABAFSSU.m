...
FREQ1 S NBV=0,DX=0,DA=$S(%V="P":%DH,1:%DB),DB=DA+$S(%V="P":3,1:PSEP),DC=DB,FN=$S(%V="P":%PH,1:%PB)+DB F UU=0:0 S DC=$N(^[QU]STRUCT(NOMG,2,%V,DC)) Q:DC=-1  Q:DC>FN  S NBV=NBV+1,DY=DC-DA X XY W ^[QU]STRUCT(NOMG,2,%V,DC)
 K EXICOM S LI=-1,TOC="^[QUW]OCUNIT(NOMG,%NORM,%IDENT)" D BLD^%VVIDEO S:'($D(NOMGC)) NOMGC=NOMG_"/COMMENT"
...
FIN S DXG=12+LCHP,DXD=79,DYH=3,DYB=22 D LWIN^%VVIDEO S DX=45,DY=12 D BLK^%VVIDEO,BLD^%VVIDEO X XY W $$^%QZCHW("UNITES") D NORM^%VVIDEO K CUM,WH,WB,MSG,NBV,DA,DB,DC,DE,FI,FN,LI,II,TOC Q
