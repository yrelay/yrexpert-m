...
;%ABANABR^INT^1^59547,73864^0
%ABANABR ;
 
PCH I $D(^EXPVU($J,PCH)) S %RL=0,VXPX=^EXPVU($J,PCH),AD="QUIT" G END
 I $E(PCH,1)="""" S PCH=$P($E(PCH,2,299),"""",1),CONCUR=0,AD="ZERO" G END
 D TO^%QZNBN1 I ISNUM=1 S AD="ESTENT" G END
 I $D(^[QUI]FULL(WB1,PCH)) S AD="FIN" G END
 
 D ^%ABRECIN I %UNK S CONCUR=0,AD="ZERO" G END
 S AD="CONT"
END Q
IND S PRF=$P(PCH,"~",1),PCHIN=$P(PCH,"~",2,999),%UC=0,PREF="^V($I,YA,"""_PRF_"."""
