...
;%ABCPGEX^INT^1^59547,73864^0
%ABCPGEX ;
 
 D KILL1 K ^ERRORGR($I),^INTERVAL($I) S DX=1,DY=3 D CLEBAS^%VVIDEO S PGEC=-1,LI=-1,%TS="   "
 F UU=0:0 S PGEC=$N(^TACOMPIL($I,PGEC)) Q:PGEC=-1  S LI=-1 F VV=0:0 S LI=$N(^TACOMPIL($I,PGEC,LI)) Q:LI=-1  D COMPR,INT^%QMDAK0 S ^[QUW]TIMCPGR(NOMG,NORM,IDENT)=^[QUW]TIMCPGR(NOMG,NORM,IDENT)_HEURE
FIN D KILL1 Q:'($D(^TACOMPIL($I)))  D ^%ABCPRE2 Q
COMPR K ^INTERVAL($I) S RGR=^TACOMPIL($I,PGEC,LI),RG=1
 ...
 F %VV="P","R" F UU=0:0 S LIG=$N(^[QUW]STRUCT(NOMG,2,%VV,LIG)) Q:LIG=-1  S NBLIG=NBLIG+1,^LIGSCRC($I,%VV,LIG)=$S('($D(^[QUW]STRUCT(NOMG,2,%VV,LIG,4))):0,^[QUW]STRUCT(NOMG,2,%VV,LIG,4)=$E($$^%QZCHW("OUI")):1,1:0) D TRAIT G:AVORT=1 END
...
AFGR K ^GROUPAR($I) S:TYPNORM="" TYPNORM=$$^%QZCHW("SANS") I $D(^[QUW]SSGROUP(GROGRI,NOMG,TYPNORM)) S SSGRO=^[QUW]SSGROUP(GROGRI,NOMG,TYPNORM) G AFGRS
...
AFGRT S:(TYPNORM=$$^%QZCHW("TOUS"))!(TYPNORM=$$^%QZCHW("TOUTES")) TYPNORM=$$^%QZCHW("SANS") S DNORM=$S(DNORM'="":DNORM,1:NORM),^[QUW]GRTYPN(NUGR,TYPNORM,DNORM)=1,^[QUW]EVALGRIL(NUGR)=EVAL,TB="^[QUW]OCGRIL(NOMG,NORM)",PDX=10,PAX=ESPC+1,DDX=((ESPC+1)*(NBC-1))+10,DP=-1,NCAZ=0
 F LL=0:0 S DP=$N(@TB@(DP)) Q:DP=-1  F XX=PDX:PAX:DDX S UPLET="" D ^%ABCPBRA
...
WAA I %VV="R" G WQQ
 S NBPAR=NBPAR+1,^[QUW]PARAGRIL(NUGR,PARA)=^LIGSCRC($I,%VV,LIG),^LIGPARA($I,%VV,LIG)=PARA,^[QUW]EVALGRIL(NUGR,EVAL)=PARA,EVAL=EVAL+1,^[QUW]PARAGRIL(NUGR,PARA,"ECR")=$S($D(^[QUW]OCUNIT(NOMG,NORM,IDENT,%VV,LIG)):$P(^[QUW]OCUNIT(NOMG,NORM,IDENT,%VV,LIG),"/",1),1:"")
...
WQR Q
AVORT D ^%VSQUEAK S AVORT=1 W !,"Comprehension avortee ! ",PARA,$S(CONCUR=0:" est inconnu",1:" est ambigu") S ^[QUW]GRILEROR(NOMG,NORM,IDENT,PARA)=$S(CONCUR=0:"Inconnu",1:"Ambigu") Q
KILL K ^[QUW]REFABAC(NOMG,NORM,IDENT),^[QUW]INTERGRI(ONUGR),^[QUW]EVALGRIL(ONUGR),^[QUW]PARAGRIL(ONUGR),^[QUW]RESUGRIL(ONUGR),^[QUW]GRILRESU(ONUGR),^[QUW]GRTYPN(ONUGR),^[QUW]ABDEJEV(ONUGR),^[QUW]GRUPLET(ONUGR)
...
ERROR S NBC=3,COL=$S(XX=10:1,XX=33:2,1:3) D PGE^%ABGRPAG S %STOP=1,^ERRORGR($I,NUGR)=""""_NOMG_""","""_NORM_""","""_IDENT_"""",^[QUW]GRILEROR(NOMG,NORM,IDENT,%ERROR)=PGE_"^"_COL_"^"_BRAQ,%ERROR=%ERROR+1
 D ^%VSQUEAK W !,"Erreur : ",BRAQ,"  Page : ",PGE,"  Colonne : ",COL G END
