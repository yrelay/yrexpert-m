...
%ABLIABG ;
 ...
 K ^POSENR($I),^TACOMPIL($I) S STOP=0,LB=20,LH=6,DX=66,DY=LB+2,PAGE=1 X XY W $$^%QZCHW("Page : "),PAGE S LI=LH,POSY=LH
LISTE S %UCI=^TABIDENT($I,"%UCI"),QUCOM=%UCI,I=1,QU=QUI,Y1=YC F UU=0:0 S Y1=$N(^[QU]SCRC(Y1)) Q:Y1=-1  D TRAIT G:STOP=1 FIN
 I QUI'=QUCOM S QU=QUCOM,Y1=YC F UU=0:0 S Y1=$N(^[QU]SCRC(Y1)) Q:Y1=-1  D TRAIT Q:STOP=1
FIN S NBPAG=PAGE D ^%ABCPGPA,@PROGTRA Q
TRAIT Q:'($D(^[QU]PRONUM(Y1)))
 S NORM=-1 F VV=0:0 Q:STOP=1  S NORM=$N(^[QU]PRONUM(Y1,NORM)) Q:NORM=-1  I NORM="GRAPHE" S IDENT=-1 F WW=0:0 S IDENT=$N(^[QU]PRONUM(Y1,NORM,IDENT)) Q:IDENT=-1  S NOMG=Y1,TYPGR=$S(QU=QUI:"(P)",1:"(C)") D GTNORM,^%ABCPGBD Q:STOP=1
END Q
GTNORM I $D(^[QUI]PRONUM(Y1,NORM,IDENT))=1 S TYPNORM=$P(^[QU]PRONUM(Y1,NORM,IDENT),"^",15) G END
 S OSTOP=STOP,NOMTAB="^[QU]PRONUM(Y1,NORM,IDENT",ADR="TYPN^%ABLIABG" D ^%VTENR S STOP=OSTOP G END
