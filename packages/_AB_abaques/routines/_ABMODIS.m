...
CHAMP D:(X1=7)!(YG=7) GOMME S ICC=IC,LL=$L(RESUL(ICC))
 S XXYYZZ=RESUL(IC),RESUL(IC)=$S(Y1'="":Y1,$D(^[QU]SCRC(SCR,IC,5)):^[QU]SCRC(SCR,IC,5),1:"") I $D(^[QU]SCRC(SCR,IC,5)) S TAB(IC)=RESUL(IC) D ^%ABAFFIU:RESUL(IC)=^[QU]SCRC(SCR,IC,5)
 S REFUSE=""
 D VERIF S STOPUC=0 G REFULOP:REFUSE=1 I $D(CMS),CMS'=1,$D(^[QU]SCRC(SCR,ICC,4)),^[QU]SCRC(SCR,ICC,4)'="" F UVU=1:1 S UCONT=$P(^[QU]SCRC(SCR,ICC,4),",",UVU) Q:UCONT=""  S IC=ICC D @UCONT G REFULOP:REFUSE=1 Q:STOPUC=1
