...
;%ABSTRUN^INT^1^59547,73864^0
%ABSTRUN ;
 
ECR S UNIT=1,ZZ="ecriture",PP=5 D DEB G:UNIT=0 NOUNIT
UTIL S ZZ="utilisation",PP=6 D DEB G:UNIT=0 NOUNIT
END K UNIT,ZZ,PP Q
DEB D POCLEPA^%VVIDEO W "unite d'",ZZ," ? (?:valeurs exist., 0:pas d'unite) "
 S AMPASY=MPASY,ALCHP=LCHP,AESPC=ESPC,(LCHP,ESPC)=LARCOL,DY=PY-%D,DX=10 X XY D ^%VLEC G:(CTRLA=1)!((CTRLF=1)!(Y1="")) FIN I C=0 S UNIT=0 G FIN
 S C=Y1 G:C="?" DEF S ^[QU]STRUCT(NOMG,2,%V,PY,PP)=C
FIN X XY W CNULI S MPASY=AMPASY,LCHP=ALCHP,ESPC=AESPC Q
DEF I '($D(^[QU]STRUCT(NOMG,2,%V,PY,PP))) X XY W CNULI G DEB
 S ADX=DX,ADY=DY D POCLEPA^%VVIDEO W "(S)uppression,(RETURN) Fin" S DX=ADX,DY=ADY X XY W ^[QU]STRUCT(NOMG,2,%V,PY,PP) R *RET K:RET=83 ^[QU]STRUCT(NOMG,2,%V,PY,PP) X XY W CNULI G DEB
NOUNIT F PP=5,6 K ^[QU]STRUCT(NOMG,2,%V,PY,PP)
 G END
DEO 
