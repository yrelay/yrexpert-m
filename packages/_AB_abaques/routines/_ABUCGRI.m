...
;%ABUCGRI^INT^1^59547,73864^0
%ABUCGRI ;
 
 S REFUSE=0,ICC=IC,REC="",LRES=$L(RESUL(IC)) F UU=1:1:LRES S REC=REC_" "
 S TAB(ICC)=REC D ^%ABAFFIU S TAB(ICC)=NOMG D ^%ABAFFIU
FIN Q
VIDE G:RESUL(IC)'="" FIN S LIMITE=15
SAUT F I1=IC:1:LIMITE S DEJALU(I1)=1,RESUL(I1)=""
 S IC=LIMITE G FIN
CHBKPT S PARA=RESUL(IC) D ^%QZCHBT S ICC=IC,TAB(ICC)=PARA D ^%ABAFFIU G FIN
DEO 
