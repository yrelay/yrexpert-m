...
;%AMREXTR^INT^1^59547,73865^0
%AMREXTR ;;07:27 PM  6 Aug 1993;
...
NXRGE S REG=$O(^[QUI]TTL(TRT,2,REG)) Q:REG=""
 ...
 I $D(@GLOSTO@("POLC")) S r=$$PARCOURS^%AMREVAL($$CONCAS^%QZCHAD(GLOSTO,"POLC"),0,3,$$CONCAS^%QZCHAD(GLOSTO,"EXPC"),EXECC)
 I r S r=$$PARCOURS^%AMREVAL($$CONCAS^%QZCHAD(GLOSTO,"POLA"),0,3,$$CONCAS^%QZCHAD(GLOSTO,"EXPA"),EXECA)
 S IRG=IRG+1
 G NXRGE
