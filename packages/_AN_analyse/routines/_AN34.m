...
FOND K @(TEMPO) S @TEMPO@(1)="C",@TEMPO@(2)="I",@TEMPO@(3)="X1",@TEMPO@(4)="X2",@TEMPO@(5)="Y1",@TEMPO@(6)="Y2" D EMPV^%ANGEPIL(TEMPO) S (NORES,X1,X2,Y1,Y2)=0
 F I=1:1:$L($$^%QZCHW("$FOND.DE.PAGE.CALENDAIRE"))+1 D LEC
 D EMPIL^%ANGEPIL("FOND1^%AN34") G EXPRESS^%AN4
FOND1 G:RESULT=0 DEPIL^%ANGEPIL S X1=NORES
 D CAR I C'=";" D M("Un ; etait attendu ..."),KILL^%ANA(X1) S RESULT=0 G DEPIL^%ANGEPIL
 S %COMPIL=%COMPIL_";" D LEC,EMPIL^%ANGEPIL("FOND2^%AN34") G EXPRESS^%AN4
FOND2 I RESULT=0 D KILL^%ANA(X1) S RESULT=0 G DEPIL^%ANGEPIL
 S Y1=NORES
 D CAR I C'=";" D M("Un ; etait attendu ..."),KILL^%ANA(X1),KILL^%ANA(Y1) S RESULT=0 G DEPIL^%ANGEPIL
 S %COMPIL=%COMPIL_";" D LEC,EMPIL^%ANGEPIL("FOND3^%AN34") G EXPRESS^%AN4
FOND3 I RESULT=0 D KILL^%ANA(X1),KILL^%ANA(Y1) S RESULT=0 G DEPIL^%ANGEPIL
 S X2=NORES
 
 S %COMPIL="d FORMAT^%PBMAGEN("_$P(%COMPIL,";",1)_","_$P(%COMPIL,";",2)_","_$P(%COMPIL,";",3)_")"
 ...
 D SET^%ANA(NORES,"TYPE","ACTION"),SET^%ANA(NORES,"TEXTE","$FOND")
