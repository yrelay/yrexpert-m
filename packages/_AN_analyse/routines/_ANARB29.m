...
;%ANARB29^INT^1^59547,73866^0
%ANARB29 ;;11:15 AM  28 Mar 1992;
 
 
 
 
 
SOLVE K @(TEMPO) S @TEMPO@(1)="SA",@TEMPO@(2)="LISTE",@TEMPO@(3)="ATTRIB",@TEMPO@(4)="PARCOURS",@TEMPO@(5)="A",@TEMPO@(6)="SA",@TEMPO@(7)="INDICE"
 
 D EMPV^%ANGEPIL(TEMPO)
 S SA=A,A=$$RED(SA,1) D EMPIL^%ANGEPIL("TOTL1^%ANARB28") G INT^%ANARBR2
TOTL1 I (VAL="")!(POS=0) D:MODAF ADD^%TLIFEN("LOG",$$^%QZCHW("  Impossible identifier la liste")) G ERR
 S LISTE=VAL
 
 
 
 S A=$$RED(SA,2) D EMPIL^%ANGEPIL("TOTL2^%ANARB29") G INT^%ANARBR2
TOTL2 I (POS=0)!(VAL="") D:MODAF ADD^%TLIFEN("LOG",$$^%QZCHW("  Impossible identifier attribut")) G ERR
