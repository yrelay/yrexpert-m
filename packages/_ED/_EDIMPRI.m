...
;%EDIMPRI^INT^1^59547,73869^0
%EDIMPRI(M) 
 
 
 
 
 N L,LMAX,PAGE,%PORT,TAB,LGMAX
...
IMPR W !,$$^%QZCHW("Variables : ") S L=L+1
 S I="" F %I=0:0 S I=$O(^[QUI]EDMOD(M,"VAR",I)) Q:I=""  D:L>(LMAX-5) TETE D IMPL(^[QUI]EDMOD(M,"VAR",I))
 W !,$$^%QZCHW("Modele    : ") S L=L+1
 S I="" F %I=0:0 S I=$O(^[QUI]EDMOD(M,"LIGNE",I)) Q:I=""  D:L>(LMAX-5) TETE D IMPL(^[QUI]EDMOD(M,"LIGNE",I))
 W # Q
 
TETE S PAGE=PAGE+1
 W #,$$^%QZCHW("Le "),$$DATE^%QMDATE,$$^%QZCHW(" a "),$$HEURE^%QMDATE
 I LGMAX=200 W ?40,$$^%QZCHW("Modele de texte : "),M G STETE
 N LTR,NBC
 S LTR=$$^%QZCHW("Modele de texte : ")_M
