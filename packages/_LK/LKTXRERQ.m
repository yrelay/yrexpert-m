...
;LKTXRERQ^INT^1^59547,74868^0
LKTXINRQ(PAR,TABATT,D,F) 
...
T S ER=$$EDGE^LKZPAR(TEMP,PARAM,PARAM1)
 ...
 Q
STO() N IND,ER,GLO,II,PARA,VAL,BASE
 S BASE=$$RP
 S ER=$$LOA^LKZPAR(TEMP,"NOM",1,.IND)
 D SX^%QSGESTI(BASE,IND)
 S PARA=$O(@ATT@(NIV,INC,""))
 F II=1:1 Q:PARA=""  D ST1 S PARA=$O(@ATT@(NIV,INC,PARA))
 D PA^%QSGESTI(BASE,IND,"DATE.RECUPERATION.INO",$$TEMPSNO^%QMDATE(DD),1)
