...
PTHM 
 ...
 S PEXP="^[QUI]PLEXI(YART",EF="EF",DED=1
L F PR=DED:1:%3NPT1 S @("V="_EF_PR),VP="V",C=PEXP_","_VP_")",(E,@("D"_PR))=$N(@C) S:@("EG"_PR)="" @("EGC"_PR)=$E(E,1) S EG=$S(@("EG"_PR)="":"EGC",1:"EG") G:(@("D"_PR)=-1)!($E(@("D"_PR),1,@("L"_PR))'=@(EG_PR)) S S @("PEXP"_PR)=PEXP,PEXP=PEXP_","_("D"_PR)
 S LIBC="" F UU=1:1:PR S LIBC=@("D"_UU)_$S(LIBC="":"",1:".")_LIBC
...
TRAIT S SCH=$P(CH,SEP,Z1),SSCH=SCH+0,@("L"_Z1)=$L(SCH) I SSCH=SCH S @("EG"_Z1)=SCH,@("EF"_Z1)=SCH-1 Q
 ...
 Q
FINREF S ADR="AFF^%LXRECAB",PARQ=PEXP,PARQS=PARQ_")",NOMTREE=$E(PARQS,1,$L(PARQS)-1) D ^%QCAGTU1
 Q
AFF S LIBD=LIBC F WW=PR+2:1:%PR S LIBD=B(WW)_"."_LIBD
...
TEST 
TEST1 W !," Abreviation a determiner : " R PCH Q:PCH=""  D ^%LXRECAB S PAR=-1 F UU=0:0 S PAR=$N(^MOTLEX($I,PAR)) Q:PAR=-1  W !,PAR
 G TEST1
