...
LEXUC7 ;
 ...
 N TLEX
 S TLEX(1)=$S($D(PCH):PCH,1:"")
 D CLEPAG^%VVIDEO,POINT^%VAJOUT("LEXIQUE","TLEX")
 S TB=$P($T(TB),";;",2) F UU=1:1 S T1=$P(TB,",",UU) Q:T1=""  S T2="^"_T1 K @(T1) S LU=-1 F %U=1:1 S LU=$N(@T2@($I,LU)) Q:LU=-1  S @T1@(LU)=@T2@($I,LU)
 ...
 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR S (IC,ICC)=OICC
FIN S CTRLF=0 Q
TB ;;FRESU
