...
D D ACCOR G:'(ACCOR) LOOP1 S OAJCMS=$S($D(%AJCMS):%AJCMS,1:""),%AJCMS="Duplication",ABENDSCR=0
 D POCLEPA^%VVIDEO,^%VSQUEAK W "N'oubliez pas de changer les champs cles" H 2 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^%VMODISC S %AJCMS=OAJCMS G LOOP1:ABENDSCR=1 D ^%VKAUSTQ S ^PRESCR($I,SCR,%NBPVU)=NOMTAB_"("_$P(NOM,"(",2,599),%PCOUR=%NBPVU,%NBPVU=%NBPVU+1 D NEWOLD G LOOP1
M D ACCOR G:'(ACCOR) LOOP1 S ABENDSCR=0,%AJCMS="Modification" D ^%VATYPA,^%VMODISC G LOOP1:ABENDSCR=1 K @(OLDNOM),@(OLDFULL) D ^%QUERKILL,^%MZAUTST S ^PRESCR($I,SCR,%PCOUR)=NOMTAB_"("_$P(NOM,"(",2,599) D NEWOLD G LOOP1
S D ACCOR G:'(ACCOR) LOOP1 D ^%VSQUEAK S PADMESSA="                          **** confirmer la suppression ****" D PAD^%VZEATT R *X1 G LOOP1:X1'=83 D OABRES^%VMODISC,^%QUERKILL K @(OLDNOM),@(OLDFULL),^PRESCR($I,SCR,%PCOUR) D CLEPAG^%VVIDEO G LOOP1
