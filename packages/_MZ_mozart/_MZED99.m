...
RD W EDBE R *IN:%BF(5) S:'($T) IN=0 Q
...
Q Q
POS I '($D(ED(EDCL))) W $$N S XOC=XOP D MS^%MZEDSF,RF D:(XOC=1)!(XOC=2) @("BLD^%VVIDEO"_$S(XOC=1:",REV^%VVIDEO",1:""))
 S:EDP>$L($$SUB(X)) EDP=$L($$SUB(X))+1 S DX=(EDP-1)#RM,DY=((EDP-1)\RM)+ED(EDCL) I DY>EDSL W $$N D MS^%MZEDSF,RF G POS
 X XY Q
...
1 D POS,W,SRF
0 D POS
R D PS0,RD G CTR:((IN'>%BF(6))!$D(^MOZED($J,IN)))!EDRP D CHR G R
E D ^%VSQUEAK S EDRP=0 Q
CE D E G @%
W W $E($$SUB(X),EDP,((EDSL-ED(EDCL))*RM)+RM) X:RM'=$X XL W $$N Q
X S ODX=$X,ODY=$Y D NORM^%VVIDEO S DX=ODX,DY=ODY X XY
 ...
 I XOP=1 S EDP=1,%=1 D REV^%VVIDEO,BLD^%VVIDEO Q
 I XOP=2 S EDP=1,%=1 D BLD^%VVIDEO Q
 Q
CR 
CR1 I (XOP=1)!(XOP=2) D ^%MZEDOP(X,XOP,"M",.X,.%AB) G E
 S ED=$O(@%TT@($J,0,EDCL)),EDN=$S(ED:(EDCL+ED)/2,1:EDCL+1) G E:(EDN=EDCL)!(ED=EDN) S ^(EDN)=$S($D(XOP):XOP,1:0)_"^"_$E(X,EDP,999),X=$E(X,1,EDP-1),^(EDN,0)=EDCL,EDCLP=EDCL,DX=EDP-1,EDP=1,%=$S(EDRP:3,1:2) D X S:ED ^(ED,0)=EDCL S XOP=3 Q
CR2 G E:EDP'=1 S EDP=1,ED=$O(@%TT@($J,0,EDCL)),EDN=$S(ED:(EDCL+ED)/2,1:EDCL+1) G E:(EDN=EDCL)!(ED=EDN) S ^(EDN)=XOP_"^"_X,XOP=3,X="",^(EDCL)=XOP_"^"_X,^(EDN,0)=EDCL,EDCLP=EDCL,DX=EDP-1,EDP=1,%=$S(EDRP:3,1:2),%=3 Q
...
DPCR G:%TAB=0 DPCR1 G:XOP>2 DPCR1 S X="",EDN=EDCL D X,^%VSQUEAK Q
...
DNCR S EDN=$O(@%TT@($J,0,EDCL)) G E:'(EDN) S EDX=$P(^(EDN),"^",2,999),ED=$L(EDX) G E:(ED+$L(X))>500 S X=X_$P(^(EDN),"^",2,999),XOP=$P(^(EDN),"^",1)
 ...
 S ^(EDCL)=$S($D(XOP):XOP,1:0)_"^"_X_$P(^(EDN),"^",2,999) S:$O(^(EDN)) ^($O(^(EDN)),0)=EDCL K @%TT@($J,0,EDN),ED(EDN) S:EDFL=EDN EDFL=EDCL S EDP=($L(X)-ED)+1,%=3 Q
...
DNL 
 ...
 G E:EDP>$L(X) S EDDL=$E(X,EDP,999),X=$E(X,1,EDP-1),%=1 Q
...
RFL0 
 I %WU=1 D REV^%VVIDEO,BLD^%VVIDEO X XY W XAF X:RM'=$X XL D NORM^%VVIDEO G RFL1
 I %WU=2 D BLD^%VVIDEO X XY W XAF X:RM'=$X XL D NORM^%VVIDEO G RFL1
 W XAF X:RM'=$X XL
...
SUB(%X1) 
 ...
 Q %X2
N() Q $C(27,$A("["),$A(0),$A("m"))
GOLD D ^%VCRIFLD($J("",42),1,21,0,22),REV^%VVIDEO,BLD^%VVIDEO,^%VCRIFLD($J("",10),1,21,0,22) W $$N D RST Q
RST I (XOP=1)!(XOP=2) D @("BLD^%VVIDEO"_$S(XOP=2:"",1:",REV^%VVIDEO"))
 Q
