...
MOZPLECT ;
 ...
 N RET I '($D(^[QUI]MOZP("VALIDATION"))) D POCLEPA^%VVIDEO,^%VSQUEAK W $$^%QZCHW("Veuillez faire au moins une validation correcte avant ... [RETURN]") R *RET:^TOZE($I,"ATTENTE") Q
...
BLOCE I IO=47 U IO S B="" F I=1:1:LIM R *C S B=B_EBCDIC(C)
 ...
 R *RET:0 D TOUCHE^%INCCLAV(RET,.RET) Q:RET'=1
...
I 
 ...
 C IO O IO:("EFL":LIM:8000) D LECTI C IO C IMP U 0
 D POCLEPA^%VVIDEO W $$^%QZCHW("Lecture terminee ("),N,$$^%QZCHW(") ... [RETURN]") R *RET:^TOZE($I,"ATTENTE")
 D POCLEPA^%VVIDEO Q
LECTI U IO W:IO=47 *-5 S STOP=0
 ...
 U IO W:IO=47 *-5 Q
BLOCI U IO S B="" F I=1:1:LIM R *C S B=B_EBCDIC(C)
 U IMP W ?0,B,!
 S N=N+1,DX=0,DY=23 U 0 X XY W $$^%QZCHW("Nombre d'enregistrements lus = "),N
 U IMP R *RET:0 D TOUCHE^%INCCLAV(RET,.RET) Q:RET'=1
...
SSF D POCLEPA^%VVIDEO S IO=$$^ZMESDATA($$^%QZCHW("Nom du fichier : "))
