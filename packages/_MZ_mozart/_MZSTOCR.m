...
;%MZSTOCR^INT^1^59547,73872^0
MOZSTOCR ;
 
 
 S:'($D(^TOZE($I,"FULL"))) ^TOZE($I,"FULL")=0 I ^TOZE($I,"FULL")=1 G FULL
 I $D(^TABIDENT(WHOIS,"INTERDIT","MOD",$I)) G FULL
 D AUTOST(.NOMTAB,.SCR,.RESUL) Q:ABENDSCR=1  S %NF1=$P(NOMTAB,"]",2),%NFU="" S:%NF1="" %NF1=$P(NOMTAB,"^",2) I $D(^NAMEFUL(%NF1)) S %NFU="^[QUI]"_$P(^NAMEFUL(%NF1),"^",1)
 G:%NFU="" COND D GTSFRESU(.RESUL,.FRESU),AUTOST(.%NFU,.SCR,.FRESU)
 K AFRESU S %12=-1 F %11=1:1 S %12=$N(FRESU(%12)) Q:%12=-1  S AFRESU(%12)=FRESU(%12) K FRESU(%12)
COND S (ARTI,WB1)=RESUL(1),TBA="^[QUI]MOZREL(WB1)",WB2=RESUL(2),WB3=RESUL(3),WB4=RESUL(4) D ^%MZCPTIN K WB1,WB2,WB3,WB4,ARTI,TBA
 K %11,%12 Q
...
REFUL D ^%VSQUEAK,POCLEPA^%VVIDEO W "Informations trop abondantes : veuillez abreger les champs " H 2
 ...
 H 2 D ^%VAFIGRI,^%VAFISCR S IC=1,ABENDSCR=0 D ^%VMODISC G MOZSTOCR:ABENDSCR=0 Q
REFUS D ^%VSQUEAK,POCLEPA^%VVIDEO W "Il faut obligatoirement une information dans  ",^%SCRE(SCR,IC,1) H 1 D ^%VAFIGRI,^%VAFISCR S ABENDSCR=1 D LOP^%VMODISC G MOZSTOCR:ABENDSCR=0 Q
