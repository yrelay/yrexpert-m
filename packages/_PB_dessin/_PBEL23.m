...
1 N L,PA,PD,T,XD,YD
 ...
 S L=$$VAL^%PBEL2(NUFA,TWREF,PAR(1),ME,.STOP) Q:(L="")!STOP
 S T=$$VAL^%PBEL2(NUFA,TWREF,PAR(2),1000,.STOP) Q:(T="")!STOP
...
2 N PA,PC,PD,RA,XA,XD,YA,YD
 ...
 S RA=$$VAL^%PBEL2(NUFA,TWREF,PAR(3),ME) Q:(RA="")!STOP
...
3 N AN,PA,PC,PD,XC,XD,YC,YD
 ...
 I '($D(POINT(PD))) D POCLEPA^%VVIDEO,^%VSQUEAK W "Point encore jamais vu : ",PD," [RETURN]" R *R:^TOZE($I,"ATTENTE") D POCLEPA^%VVIDEO Q
 S AN=$$VAL^%PBEL2(NUFA,TWREF,PAR(3),1000) Q:(AN="")!STOP
 S I=POINT(PD),XD=$P(I,"^",1),YD=$P(I,"^",2)
 D ROTP^%PBCACV(XC,YC,XD,YD,AN,.X,.Y) G OK
 
OK S:N'="" POINT(N)=X_"^"_Y
 
 D:AFF CALXY^%PBCACV(X,Y,XT,YT,ROT,AGR,.X,.Y),POINT^%PBMFN(X,Y,0)
