...
;%PBIAELL^INT^1^59547,73873^0
PTIAELLI ;
 N A,ATT,B,C,D,GR,I,IN,PAR,PC,PD,PO,SPR,PR,R,R1,R2,SN,SY,XA,XC,XHA,XD,XHD,XR,YA,YC,YHA,YD,YHD,YR,ADRES S ADRES=1
...
L0 D ^%PBPZGRA S R=$$LOC^%PBMFN(.XR,.YR) I R=0 D EFF3 G FIN
 ...
 D ELLIPSE^%PBMFN(XC,YC,PR,GR,I,0,TYPL,VISIBL)
S0 S ADRES=$$CONFIRM^%PBMEN($$^%QZCHW("syntaxe")_" (O/N) : ")
 I ADRES G 1
 S (PD,PO,IN,R1,R2,PC)=""
 G END
1 
 S SPR=PR D ^%PBPZALF,CLEPAG^%VVIDEO W "Syntaxe : " S DX=$X,DY=$Y D ^%VLEC
 S PR=SPR
...
S1 S PD=PAR(1),PO=PAR(2),R2=PAR(3)
 ...
 D CAL,ELLIPSE^%PBMFN(XC,YC,PR,GR,I,0,TYPL,VISIBL) G END
S2 I MODE D MODE^%PBIZERR G L0
 ...
 D ELLIPSE^%PBMFN(XC,YC,PR,GR,I,0,TYPL,VISIBL) G END
END S R=$$ADD^%PBSOV(O,V,$$ELLIPSE^%PBEL(XC,YC,PR,GR,I,TYPL,PD,PO,PC,R1,R2,IN_"^"_Y1))
...
EFF4 D EFF1,EFF2,ELLIPSE^%PBMFN(XC,YC,PR,GR,I,1,TYPL,VISIBL) Q
