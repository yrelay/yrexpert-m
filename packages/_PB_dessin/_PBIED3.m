...
POINTS(NBASE,IND,O,V,OB,PTS,PARAM,STOP) 
 D INT(NBASE,IND,O,V,0,0,0,1,PTS,0,PARAM,OB) Q
SOFT3(NBASE,IND,O,V,OB,XT,YT,ROT,AGR,PTS,PARAM,STOP,AFF) 
 D INT(NBASE,IND,O,V,XT,YT,ROT,AGR,PTS,AFF,PARAM,OB) Q
SOFT2(NBASE,IND,O,V,OB,XT,YT,ROT,AGR,PTS,PARAM,STOP) 
 D INT(NBASE,IND,O,V,XT,YT,ROT,AGR,PTS,1,PARAM,OB) Q
SOFT(NBASE,IND,O,V,XT,YT,ROT,AGR) N OBB,PB1,PB2,STOP
 D INT(NBASE,IND,O,V,XT,YT,ROT,AGR,"PB1",1,"PB2","OBB") Q
INT(NBASE,IND,O,V,XT,YT,ROT,AGR,PTS,AFF,PARAM,OBJET) N EL,M,POINT,R
 S M=^[QUI]GRAPHE2(O,V,0,"MESURE"),STOP=0
 F EL=1:1:^[QUI]GRAPHE2(O,V) D ^%PBIED4(NBASE,IND,O,V,EL,XT,YT,ROT,AGR,M) Q:STOP  R *R:0 D:R=1 ERR Q:STOP
 Q
ERR D ^%PBPZALF
