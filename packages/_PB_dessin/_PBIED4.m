...
;%PBIED4^INT^1^59547,73873^0
PTIGDX4(NBASE,NIND,O,V,EL,XT,YT,ROT,AGR,ME) 
 N CH
 S CH=^[QUI]GRAPHE2(O,V,EL) D @("^%PBIED4"_$P(CH,"^",1)) Q
 
VAL(NBASE,NIND,PAR,ME,AB) 
 S:NIND="" NIND=0 N PD,R,S,SI,S2,VAL S AB=0
 S SI="+" I "+-"[$E(PAR,1) S SI=$E(PAR,1),PAR=$E(PAR,2,$L(PAR))
 S PCH=PAR D TO^%QZNBN1 I ISNUM S VAL=PAR G OKV
 I $D(@PARAM@(NIND,PAR)) S VAL=@PARAM@(NIND,PAR) G OKV
 G:NBASE="" ASK G:'($$AIR^%QSGEST5(NBASE,NIND,PAR)) ASK
 S VAL=$$^%QSCALVA(NBASE,NIND,PAR) G OK
ASK D ^%PBPZALF
