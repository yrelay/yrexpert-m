...
1 
 ...
 S PR=$$VAL^%PBIED4(NBASE,NIND,R1,ME,.STOP) Q:(PR="")!STOP
 S GR=$$VAL^%PBIED4(NBASE,NIND,R2,ME,.STOP) Q:(GR="")!STOP
 S I=$$VAL^%PBIED4(NBASE,NIND,IN,1000,.STOP) Q:(I="")!STOP
 G OK
2 
...
OK D CALXY^%PBCACV(X,Y,XT,YT,ROT,AGR,.X,.Y)
 ...
 S I=$$A^%PBCACV(I,ROT)
 D ^%PBPZGRA,ELLIPSE^%PBMFN(X,Y,PR,GR,I,0,TL,0) Q
