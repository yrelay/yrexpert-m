...
INIT D INIT^%PBP5EC N X,Y
 S X=$S($D(^[QUI]PTPOSIT("X",$I)):^[QUI]PTPOSIT("X",$I),1:^[QUI]PTPOSIT("X"))
 S Y=$S($D(^[QUI]PTPOSIT("Y",$I)):^[QUI]PTPOSIT("Y",$I),1:^[QUI]PTPOSIT("Y"))
 D ORIG^%PBP5EC2(X,Y) Q
 
FIN D FIN^%PBP5EC Q
 
 
POINT(X,Y,E) D POINT^%PBP5EC($$X(X),$$Y(Y),E) Q
...
HORTO(X1,Y1,X2,Y2,XH1,YH1,XH2,YH2,CX,CY,C) N MAXX,MAXY,MINX,MINY
 ...
 S Y=-(C) I Y>MAXY S YH1=MAXY,XH1=((-(MAXY))-C)/CX G S1
 I Y<MINY S YH1=MINY,XH1=(-(C))/CX G S1
 S XH1=MINY,YH1=Y
S1 S Y=((-(CX))*MAXX)-C I Y>MAXY S YH2=MAXY,XH2=((-(MAXY))-C)/CX Q
 I Y<MINY S YH2=MINY,XH2=(-(C))/CX Q
 S XH2=MAXX,YH2=Y Q
