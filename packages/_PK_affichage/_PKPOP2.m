...
BEG 
 ...
 S TB=$P(Y,"\",6),TF=$L(TB,","),TD=((TF/TC)+9E-1)\1 F TI=1:1:TF S Y(TI)=$P(TB,",",TI),TJ(((TI-1)\TD)+1,(80+(SER*2))-$L(Y(TI)))=""
 G SET
Y I $D(Y(0)) S TA=Y(0)
 E  F TA=1:1 I $D(Y(TA))=0 S TA=TA-1 Q
 S TI=0,TD=((TA/TC)+9E-1)\1 S:TD["." TD=(TD+9E-1)\1
 F TJ=1:1:TC S TG=TI+TD S:TG>TA TG=TA F TI=TI+1:1:TG S TJ(TJ,(80+(2*SER))-$L($P(Y(TI),"*")))=""
...
COL S TW=0,%C=TF+1 F TJ=1:1:TC S TW=(TW+82)-$O(TJ(TJ,"")),TC(TJ)=%C,%C=(TF+1)+TW
 ...
 ;HL002 S %C=TF I TP["L" S TE=PK("VL")_$J("",TW)_PK("VL"),TM=PK("HL")_PK("HL") D:TP["T" 1,2,3 D:TP'["T" 3,4,1 G LIST
 S %C=TF I TP["L" S TE=PK("VL")_$J("",TW)_PK("VL"),TM=PK("HL")_PK("HL") W PK("SET") D:TP["T" 1,2,3 D:TP'["T" 3,4,1 G LIST
...
LIST
 I $L($P(Y,"\",4)) S %C=(((TW-$L($P(Y,"\",4)))\2)+%C)+1,%R=TT W @(PKP),$P(Y,"\",4)
 S TM=0,TE="TA="_PKP,TH=""
 F TJ=1:1:TC S %C=TC(TJ),%R=TT F TM=TM+1:1:TM+TD Q:$D(Y(TM))=0  S %R=%R+1,@(TE),TX=$S(SER:"",1:" ")_Y(TM),CA=$E(TX,$L(TX)) S:CA'="*" TB(TM)=TA_TX_$S(SER:"",1:" "),TH=TH_$$MAJ^%VMINUSC($E(TX,1,15))_"\" S:CA="*" TX=$P(TX,"*"),TH=TH_"\" W TA,TX
...
SEL0 G:X>TI SEL00
 ...
 I (TE<27)&(TE>0) S CTR=$C(TE+64) G SEL1
 D:$D(TB(X))=0 TT
 G SEL0
SEL00 S X=$O(TB(0))
 G SEL0
