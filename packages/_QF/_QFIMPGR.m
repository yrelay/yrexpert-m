...
;%QFIMPGR^INT^1^59547,73875^0
GRIMPGRO ;
 
 S DY1=$Y I '($D(^PAD($J,PAD,DY1))) D ^%VSQUEAK G END2
 S GROIMP=^PAD($J,PAD,DY1) D ZD^%QMDATE4,^%QMDAHEU K LGMAX,^AFFPAG($J)
AA S STR="@E#cran ou @I#mprimante ?" D ^%QFOPTIO R *X G:($C(X)'="E")&($C(X)'="I") END1 S OPTION=$C(X),NBML=$S(OPTION="E":21,'($D(^TABIDENT(WHOIS,"IMPRCOUR"))):60,1:^TABIDENT(WHOIS,"IMPRCOUR"))
 D POCLEPA^%VVIDEO,BLK^%VVIDEO W "Un instant..." D NORM^%VVIDEO
 S %TABCH="^[QUI]GROUPE(NUFA,1,GROIMP,""OBJET"")" D ^%QFLGIND S LGMAX0=%LGMAX
...
LOOP1 S OBJ=$N(^[QUI]GROUPE(NUFA,1,GROIMP,"OBJET",OBJ)) G:OBJ=-1 FL1 F I=1:1:NBAT S VALAT=$$PI^%QFCALVA(OBJ,^ATTR($J,"O",I)) I $L(VALAT)>LGMAX(I) S LGMAX(I)=$L(VALAT)
FL1 S LGTOT=LGMAX0 F I=1:1:NBAT S LGTOT=(LGTOT+LGMAX(I))+3
 ...
 S ^AFFPAG($J,EGAP,LIGN)="NOMBRE "_%NINDS_": "_^[QUI]GROUPE(NUFA,1,GROIMP,"CARD"),LIGN=LIGN+1
 ...
 S OBJ=-1
LOOP2 S OBJ=$N(^[QUI]GROUPE(NUFA,1,GROIMP,"OBJET",OBJ)) G:OBJ=-1 IMPR D:LIGN=NBML PAGE2 D LIGNE2 G LOOP2
LIGNE2 S A=OBJ,LONG=LGMAX0+3,NBL=LONG-$L(A) D BLANC S A=A_BLANC
 F K=1:1:NBAT S A=A_$$PI^%QFCALVA(OBJ,^ATTR($J,"O",K)),LONG=(LONG+LGMAX(K))+3,NBL=LONG-$L(A) D BLANC S A=A_BLANC,^AFFPAG($J,EGAP,LIGN)=A
 S LIGN=LIGN+1 Q
...
IMPR F P=1:1:EGAP S ^AFFPAG($J,P,1)=^AFFPAG($J,P,1)_"  (p "_P_"/"_EGAP_")"
 ...
 I OPTION="E" D 132^%VVIDEO,^%QFOPAGI,80^%VVIDEO,ENTETE^%QFECRAN,AFFGD^%QFECRAN S RETOUR=1 Q
 S IMPR=^TABIDENT(WHOIS,"PRINTER"),NBCOL=$S(LGTOT<81:80,1:132),STR="Impression @[RETURN]# " D ^%QFOPTIO R *RET O IMPR U IMPR
