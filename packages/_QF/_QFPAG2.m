...
PLCPG G:'($D(^[QUI]GROUPE(NUFA,"LISTE2",I))) FLCPG D TOP^ETOILE G:(ARRET'="FIN")&(PAG=NBPAG) FLCPG S PAG=PAG+1,LI=6 S:ARRET="FIN" NBPAG=PAG G LCPG
...
AFLIG S DX=3,DY=LI X XY W "                                    " X XY W ^PAG($J,PAG,LIG) I ^ATTR($J,"O",ATAF)="AFFECTATION" S A="^[QUI]GROAFF(NUFA,^PAG($J,PAG,LIG))" W " (",$S('($D(@A)):"sans",1:@A),")" Q
 W:^ATTR($J,"O",ATAF)'="" " (",$S($D(^[QUI]QUERY3(QUER,^PAG($J,PAG,LIG),1,^ATTR($J,"O",ATAF))):$O(^[QUI]QUERY3(QUER,^PAG($J,PAG,LIG),1,^ATTR($J,"O",ATAF),"")),1:"?"),")" Q
AFLID S DX=43,DY=LID X XY S:^ATTR($J,"O",6)="" ATTR=^[QUI]GROUPE(NUFA,1,%GR,"CARD") S:^ATTR($J,"O",6)'="" ATTR=^ATSG($J,%GR) W %GR," (",ATTR,")" Q
