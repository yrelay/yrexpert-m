...
0 Q DATE
1 S COR=$$^%QMDASAI($$INVERS^%QMDATE(DATE),.DATE) Q DATE
2 S COR=$$^%QMDASAI(DATE,.DATE) Q DATE
3 S COR=$$^%QMDASAI($$SLASH^%QMDATE(DATE),.DATE) Q DATE
4 G 2
5 G 2
6 Q $$^%QMDAFIN(DATE)
61 Q $$^%QMDAFIN("S"_$E(DATE,3,4)_"/"_$E(DATE,1,2))
7 Q $$^%QMDAFIN(DATE)
8 Q $$^%QMDAFIN(DATE)
9 Q $$^%QMDAFIN(DATE)
10 Q $$^%QMDAFIN(DATE)
101 S D=$P(DATE," ",1)_" "_$$INVERS^%QMDATE($P(DATE," ",2))
 S COR=$$^%QMDASAI(D,.DATE) Q DATE
102 G 2
103 S D=$P(DATE," ",1)_" "_$$SLASH^%QMDATE($P(DATE," ",2))
 S COR=$$^%QMDASAI(D,.DATE) Q DATE
104 G 2
105 G 2
1001 S COR=$$^%QMDASAI($$INVERS^%QMDATE(DATE),.DATE)
 Q DATE
1002 G 2
1005 G 2
1006 G 2
...
21104 G 2
21105 G 2
21106 S D=$$SLASH^%QMDATE($E(DATE,1,6))_" "_$E(DATE,7,8)_"H"_$E(DATE,9,10)_"'"_$E(DATE,11,12)
 S COR=$$^%QMDASAI(D,.DATE) Q DATE
TEST D CLEPAG^%VVIDEO
DEB W !,$$^%QZCHW("Date : ") R D Q:D=""
 W !,$$^%QZCHW("Format : ") R F G:F="" DEB
 W !,$$^%QZCHW("Forme interne : "),$$^%QMDAINT(D,F),! G DEB
