...
NOSEM(H) 
 N NS,A,HPS,HJ,DHJ,PLUS,AJ
 S A=$$ANNEEX^%QMDATE(H),HPS=$$HPS(A)
 ;HL S HJ=HPS-4,DHJ=$$DATENO3^%QMDATE(HJ),AJ=$E(DHJ,1,2),PLUS=$S(AJ=A:2,1:1)
 S HJ=HPS-4,DHJ=$$DATENO3^%QMDATE(HJ),AJ=$E(DHJ,1,4),PLUS=$S(AJ=A:2,1:1)
 I (HPS>H)&(PLUS=1) Q $$NOSEM($$20^%QMDATE1((A-1)_1231))
 I HPS>H S NS="0"_1 Q A_NS
 S NS=((H-HPS)\7)+PLUS S:$L(NS)<2 NS="0"_NS
 Q A_NS
HLUN(NSC) 
 N HPS,HJ,A,DHJ,AJ
 S A=$E(NSC,1,2),HPS=$$HPS(A),HJ=HPS-4,DHJ=$$DATENO3^%QMDATE(HJ),AJ=$E(DHJ,1,2)
 I AJ'=A Q HPS+(7*($E(NSC,3,4)-1))
 Q (HPS-7)+(7*($E(NSC,3,4)-1))
HPS(A) 
 N H1,J
...
BIS2 S HH=$$HLUN(SEM),DAT=$$01^%QMDATE1(HH) Q
