...
QMEANOE ;
 ...
 N ADRES,CONTR,LCOUR,FEN,TOTO,TEMP1,TEMP,N,GLOB,NOE
 ...
 S @TOTO@(6)=$$^%QZCHW("Imprimer")_"^IMPRIM"
 S @TOTO@(6,"COM")=$$^%QZCHW("Impression de noeuds du reseau")
 S @TOTO@(7)=$$^%QZCHW("Page")_"^PAGE"
 S @TOTO@(7,"COM")=$$^%QZCHW("Changer de page")
 S @TOTO@(8)="+^PLUS",@TOTO@(8,"EQU")="="
 S @TOTO@(8,"COM")=$$^%QZCHW("Avancer d'une page")
 S @TOTO@(9)="-^MOINS",@TOTO@(9,"EQU")="_"
 S @TOTO@(9,"COM")=$$^%QZCHW("Reculer d'une page")
 S @TOTO@(10)=$$^%QZCHW("Repere")_"^REPER"
 S @TOTO@(10,"COM")=$$^%QZCHW("Voir le repere des pages")
 S @TOTO@(11)=$$^%QZCHW("Changer le nom d'un noeud")_"^CHANGN"
 S @TOTO@(11,"COM")=$$^%QZCHW("Changer le nom d'un noeud du reseau")
 S LCOUR="" D PFEN
AFF2 D REAFF
AFF3 
...
CADLL D CARSP^%VVIDEO(40,0,3,40,1)
 S LCOUR=WHOIS_$$^%QZCHW(" sur ")_$I,DX=41,DY=1 X XY W $J("",38)
...
DUPLI S NOE=$$UN^%QUAPAGM G:NOE="" AFF2
 D:$$DUP^%QMEANOG(NOE) REINIT
 G AFF2
 
CHANGN S NOE=$$UN^%QUAPAGM G:NOE="" AFF2
 D:$$CHGN^%QMEANOG(NOE) REINIT
 G AFF2
IMPRIM 
 D ^%QMEANPR G AFF3
SUPP 
