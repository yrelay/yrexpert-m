...
;%QNEQM2^INT^1^59547,73879^0
QMEQM2(LISTIND,LLISTEN,TEMPS,CYCLE,STO,R,MODE,DEB,INC,INCOMPG,INSEP,INS) 
 ...
 F I=1:1 D MORE(STO,K,NMIN,LISTIND,BASE,@R@("LLISTEN"),TEMPS,R,CARD,I,INC,INCOMPG,INSEP,INS) Q:'($$^%QNEQM(STO,CARD,K,MODE))
...
MORE(STO,K,NMIN,LISTIND,BASE,LLISTEN,TEMPS,R,CARD,N1,INC,INCOMPG,INSEP,INS) 
 N EQUI,N,CMAX,MAXT,EQUIR,NTOT,MSG
 S MSG=""
 D PRINT^%QNEQM(STO,K,N1)
 
 I '($$OKPART(STO,K,NMIN)) Q
 
 D LLISTEN(STO,K,LISTIND,BASE,LLISTEN,0)
 ...
 D INT^%QNEQUI6(0,LLISTEN,CYCLE,TEMPS,.EQUI,.N,.CMAX,.MAXT,.EQUIR,.NTOT)
 W " equilibre reel ",EQUIR," equilibre reel max ",@R@("EQUIRM")
 I @R@("EQUIRM")>EQUIR D FIN Q
...
INT(NOM,L,TEMPS,INC,INCOMPG,INSEP,INS,CYCLE,TAL,TAG,LLISTEN,DATE,COMM,MODE,MODAF,MSG) 
 ...
 K @(ADR)
 S DEB=1
 D QMEQM2(L,.LLISTEN,TEMPS,CYCLE,STO,R,1,DEB,INC,INCOMPG,INSEP,INS)
 Q
ADR() Q $$CONCAS^%QZCHAD("^TEMPORAY","A,QMEQM")
 
TEST S LISTIND=$$^%QSCALVA($$REP^%QNEQETU,"EQL88080008","LISTE.OPERATIONS.GAMME")
 S TEMPS="TEMPS.GENERE",CYCLE=600
 S STO="STO1",R="R1"
 K @(STO),@(R)
 S DEB=1
 D QMEQM2(LISTIND,TEMPS,CYCLE,STO,R,1,DEB)
 Q
...
PRINTL(LISTIND) 
 ...
 W !,LISTIND
 S IND=$O(@ADRLT@(""))
 F I=0:0 Q:IND=""  W !,IND," ",@ADRLT@(IND) S IND=$O(@ADRLT@(IND))
 Q
