...
ACT N ADRES,CONTR,LCOUR,FEN,TATA,TOTO D ^%QS3INIT
 S TOTO="21^10^79"
 S TOTO(1)=$$^%QZCHW("Entrer")_"^ENTREE"
 S TOTO(1,"COM")=$$^%QZCHW("Entrer dans la fenetre")
 S TOTO(2)=$$^%QZCHW("Histogramme simple")_"^GRAPH1"
 S TOTO(2,"COM")=$$^%QZCHW("un attribut de totalisation et un de regroupement")
 S TOTO(3)=$$^%QZCHW("Histogrammes paralleles")_"^GRAPH2"
 S TOTO(3,"COM")=$$^%QZCHW("deux attributs de totalisation et un de regroupement")
 S TOTO(4)=$$^%QZCHW("Histogrammes serie")_"^GRAPH3"
 S TOTO(4,"COM")=$$^%QZCHW("un attribut de totalisation et deux de regroupement")
 S TOTO(9)=$$^%QZCHW("Supprimer")_"^SUPPR"
 S TOTO(9,"COM")=$$^%QZCHW("Supprimer des listes")
 S TOTO(5)="+^PLUS",TOTO(8,"EQU")="="
 S TOTO(5,"COM")=$$^%QZCHW("Avancer d'une page")
 S TOTO(6)="-^MOINS"
 S TOTO(6,"COM")=$$^%QZCHW("Reculer d'une page")
 S TOTO(7)=$$^%QZCHW("Page")_"^PAGE"
 S TOTO(7,"COM")=$$^%QZCHW("Changer de page")
 S TOTO(8)=$$^%QZCHW("Repere")_"^REPER"
 S TOTO(8,"COM")=$$^%QZCHW("Afficher le repere des pages")
 S LCOUR="" D PFEN
AFF2 D REAFF
AFF3 
ACT2 S DX=0,DY=22 D CLEBAS^%VVIDEO,POCLEPA^%VVIDEO
...
GRAPH1 S R=$$^%QS0G3 G AFF2:R=2,AFF3:R=1,ACT2:1
GRAPH2 S R=$$^%QS0G5 G AFF2:R=2,AFF3:R=1,ACT2:1
GRAPH3 S R=$$^%QS0G4 G AFF2:R=2,AFF3:R=1,ACT2:1
...
IFEN(FEN) S FEN("C",1,"T")=$$^%QZCHW("Card."),FEN("C",1,"L")=6,FEN("C",1,"C")="CARD",FEN("C",2,"T")=$$^%QZCHW("Repertoire"),FEN("C",2,"L")=9,FEN("C",2,"C")="OBJET"
 ...
 I $D(^DERLISTE($I)),^DERLISTE($I)'="" D AFFP^%QUAPAGM(^DERLISTE($I))
 Q
