...
;%QSGORE2^INT^1^59547,73882^0
QSGOREQ(REQ,BASE) ;;05:30 PM  4 Apr 1989
...
S0 I $D(^[QUI]RQSNOLOG(Y1)) S ENTREEW=$$LISTIND^%QSGEST6($$NOMINT^%QSF(Y1)) G:Y1=BASE GO D ^%VZEAVT($$^%QZCHW("Cette liste ne contient pas des ")_BASE_$$^%QZCHW("s ")) G L0
 ...
 G:$$^%QSCALVA("L0",Y1,"BASE")=$$NOMINT^%QSF(BASE) GO
...
GO S ENTREE=Y1,RQSRESUL=$$GEN^%QCAPOP("RQS")
 ...
 S (STOPCALC,ERRCAL)=0 D CALCUL G:ERRCAL FIN G:STOPCALC BATCH
 ...
 D PA^%QSGESTI("RQS",REQ,"ACTIVATIONS",^[QUI]RQS1(REQ,"ACTIVATIONS"),1)
...
CALCUL 
 ...
 I ('(RPAR))!ABORT S RQSRESUL="",ERRCAL=1,STOPCAL=1 Q
 S ERRCAL=0,LISTRI=$$GEN^%QCAPOP("TRI")
 I '($D(CONT)) D ^%QSRECOP Q
 I '($$^%QSCALCU(REQ,BASE,ENTREEW,RQSRESUL,LISTRI,.CARD)) S RQSRESUL="",ERRCAL=1,STOPCALC=1 Q
...
MAJPARAM 
 ...
 Q
LECPAR N Y D POCLEPA^%VVIDEO S Y=$$^%VZAME1(PAR2)
 I CTRLA S ABORT=1 Q
...
MAJNREQ 
 ...
 F %I=1:1 S PAR=$O(@TEMPOPAR@(PAR)) Q:PAR=""  S VAL=@TEMPOPAR@(PAR),COMREQ=COMREQ_"/"_$TR(PAR,".","_")_"="_VAL
 Q
