...
TL2 S CH=^%SCRE(SCR,CHAMP,"LIEN",REP),R1=$P(CH,",",1),R2=$P(CH,",",5)
 ...
 I TYP=1 D ADDO^%QSGEST3(R2,IND2,NOR1,LIAT,R1,$$NOMEX^%QSGES10(R1,IND1)) Q
 I TYP=2 D ADDO^%QSGEST3(R1,IND1,NOR2,LIAT,R2,$$NOMEX^%QSGES10(R2,IND2)) Q
 D ADDS^%QSGEST3(R1,IND1,LIAT,R2,IND2) Q
...
TREP S CH=^%SCRE(SCR,CHAMP,"QUERY",REPZ)
 ...
 I CAS=0 S VAL=RESUL(CHAMP)
 
 I CAS=1 S ATT=RESUL(CHAMP),VAL=RESUL(CHAMP+1)
 I CAS=2 S SUITE=$E(ATT,2,$L(ATT)),ATT=$S(SUITE?.N:RESUL(SUITE),1:""),VAL=$S($E(ATT)?1U:RESUL(CHAMP),1:"")
 ...
 D PA^%QSGESTI(REP,IND,ATT,VAL,NOR) Q
...
BIDF D LEX Q:LEX="" IND
 I NLEX S IND=IND_RESUL(LEX) Q:RESUL(LEX)="" "" G BIDF
