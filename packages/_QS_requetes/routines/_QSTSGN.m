...
;%QSTSGN^INT^1^59547,73883^0
%QSTSGN ;;12:02 PM  21 May 1992;
 
QSGEO N NPOLY,MPOLY,NESP,NPO,MESP,NQ,NT
 W !,"Nombre de polyedre : " R NPOLY
 W !,"Indice du premier polyedre : " R MPOLY
 W !,"Nombre d'espace : " R NESP
 W !,"Indice du premier espace : " R MESP
 W !,"Nombre de points (au minimum 441 sont crees) : " R NPO
 D CLEPAG^%VVIDEO
 W !,"Creation des points..." D BQS(NPO)
 W !,"Creation des polyedres et des figures..." D QSPOLY(NPOLY,MPOLY)
 W !,"Creation des espaces..." D QSESP(NESP,MESP)
 W !,"Creation des liens..." D LIENPOFI,LIENPOES
 Q
...
QSPOLY(N,M) 
 ...
 I N<1 W !,"Impossible cra le nombre de polyedres est < 1" Q
...
NOMFIG 
 ...
 Q
CREPOLY 
 D PA^%QSGESTI("POLY",NOMPOLY,"NOM",NOMPOLY,1) W !," "_NOMPOLY
 D PA^%QSGESTI("POLY",NOMPOLY,"NOMBRE.SOMMETS",NBRESOM,1)
 D PA^%QSGESTI("POLY",NOMPOLY,"TAILLE",TAILLE,1)
 Q
CREFIG 
 D PA^%QSGESTI("FIGU",NOMPOLY_","_NOMFIG,"NOM",NOMFIG,1) W !," "_NOMPOLY_","_NOMFIG
...
QSESP(N,M) 
 ...
 I N<1 W !,"Impossible cra le nombre d'espaces est < 1" Q
 F I=M:1:(N+M)-1 S NOMESP="ESP."_I,%R=$R(2),METR=$S(%R=0:"EUCLIDIENNE",%R=1:"FRIEDMAN"),DIM=$R(4)+1 D CREESP
 Q
CREESP 
 D PA^%QSGESTI("ESPA",NOMESP,"NOM",NOMESP,1) W !," "_NOMESP
...
CHOIX1 S %R=$R(NPO)+1
 ...
 D ADD^%QSGEST3("POINT",POINT,POINT,"SOMMET.DE","FIGU",FIG,FIG) W !,FIG_"<-->"_POINT
 Q
LIENPOES 
...
CHOIX2 S %R=$R(NESP)+MESP
 ...
 D ADD^%QSGEST3("POLY",POLY,POLY,"ESPACE.DE","ESPA",ESPACE,ESPACE) W !,POLY_"<-->"_ESPACE_" %R = "_%R
 Q
...
TRAIT1 S NOM="P"_$E("000",1,4-$L(NUM))_NUM,P=$R(101),COUL=$S(P<5:"JAUNE",P<15:"",P<25:"BLANC",P<40:"BLEU",P<65:"ROUGE",P<101:"RVB") W NOM_" "
