...
SIGN D POCLEPA^%VVIDEO S Y1=$$^%VZAME1($$^%QZCHW("Signataire : "))
...
FIN D END^%QUCHOIP Q
CAD S DXG=0,DXD=39,DYH=0,DYB=2,GRAPH=1,BIG=0,MSG=$$^%QZCHW("VISER LES VISAS")
 D 4^%VZCD Q
...
INT(R,I,S,XG,YH,HA,LA) 
 ...
 D INIT^%QUCHOI6("LVISA",1,"",XG,YH,LA,HA),AFF^%QUCHOI6
VISA S VISA=$$UN^%QUCHOI6 I VISA'="",'($$VISE(S,VISA)) D ^%VZEAVT($$^%QZCHW("Vous n'etes pas autorise a viser ce VISA")) G VISA
 D END^%QUCHOI6 Q:VISA=""
...
CODE D ECHOFF^%VVIDEO R CODE D ECHON^%VVIDEO
 ...
 D VISER($$NOMINT^%QSF(R),I,VISA,S)
 D POCLEPA^%VVIDEO W $$^%QZCHW("Visa vise ...") H 2 Q
...
VISE(S,V) 
 ...
 I '($D(^[QUI]RQSVDESC(V))) Q 0
 N CH,I,OK,SS
 S CH=^[QUI]RQSVDESC(V)
 S OK=0 F I=8:1:31 S SS=$P(CH,"^",I) I SS'="" S OK=SS=S Q:OK
 Q OK
 
VISER(REP,I,VISA,SIGN) 
 ...
 D PA^%QSGESTI(REP,I,VISA,SIGN,1)
 D PA^%QSGESTI(REP,I,VISA_".DATE",$$DATE3^%QMDATE,1)
 D PA^%QSGESTI(REP,I,VISA_".HEURE",$$HEURE^%QMDATE,1)
 Q
