...
;%QULYSTE^INT^1^59547,73886^0
ZLISTE(XG,YH,DH,DV,LI,P1,COA,COS) 
 N HELP,MODP,CP
 S HELP="^%QULYSTH",MODP=1 G GO
ZLISTE2(XG,YH,DH,DV,LI,P1,COA,COS,HELP) N MODP,CP
 S MODP=1 G GO
ZLISTE3(XG,YH,DH,DV,LI,P1,COA,COS,MODP) N HELP,CP
 S HELP="^%QULYSTH" G GO
ZLISTE4(XG,YH,DH,DV,LI,P1,COA,COS,HELP,CP) N MODP
 S MODP=1 G GO
...
OK7 D EFF S CARD=CARD-1
 ...
 S I=$ZP(@LI@("")) K:I'="" @LI@(I) K:(I'="")&$D(CP) @CP@("COMP",I) D REAF(P1C,LI) G LP
...
AFF(A,V) 
 S @LI@(P1C,A)=V Q
RECOP(S,D) 
 N I,%I S @LI@(D)=@LI@(S) D:$D(CP) ^%EDCGELI S I=-1 F %I=0:0 S I=$N(@LI@(S,I)) Q:I=-1  S @LI@(D,I)=@LI@(S,I)
 Q
