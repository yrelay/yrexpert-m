...
SATURE(ADRET) 
 N REG,GCOMP,ICOND,ADDIA,OKCONT,GREL
 N %REP,%IND,REGMC,TYPDIA,ERR,PILE,LEVEL,TEMP,ASC,%ASC
 
 S GREL="^[QUI]RQSDON(""QW"",""CD"")"
 
 
 I ISTP=0 S @ADRET="FIN^%QWAC" Q
 
 
 I @WWSTEP@(ISTP,"RETRY") D RECUSE^%QWMCACS S @ADRET="NEXREG^%QWAC" Q
...
UNXDOM I DOMCOUR'="GENERAL" S DOMCOUR=^[QUI]RQSDON("QW","CD",DOMCOUR,"PERE"),@ASC@(%ASC)=DOMCOUR,%ASC=%ASC-1 G UBCDOM
 ...
 S %REP=""
DNXREP S %REP=$O(@WWSTOCK@(%REP)) G:%REP="" RETRY^%QWMCACS
 S %IND=""
...
ACTION(DOM,REG) 
 ...
 I VERBE="DIALO" S TYPDIA="SATU",RES="DIALO^%QWMCACS"
 Q RES
