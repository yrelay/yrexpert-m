...
;%SDESDEL^INT^1^59547,73889^0
SDESDEL(XD,YD,XA,YA,XR,YR,TYPL,VIS,OK,OBLI) 
 
 
 
 
 N A,ATT,B,C,D,GR,I,IN,PAR,R,R1,R2,XHA,XHD,XC,YHA,YHD,YC
 S OK=1
 S TYPL=$$TYPL^%SDEDEF($$ZSUBST^%QZCHSUB(TYPL,"""",""))
 D EFFBAS^%SDUMEN,AFFI^%SDUMEN(0,0,$$^%QZCHW("Pointez le point de depart de la demie-ellipse avec la mire"))
L0 D ^%PBPZGRA S R=$$LOC^%PBMFN(.XD,.YD)
 I R=0,OBLI G L0
 D EFFBAS^%SDUMEN
 I R=0 S OK=0 G FIN
 D POINT^%PBMFN(XD,YD,0)
 D AFFI^%SDUMEN(0,0,$$^%QZCHW("Pointez le point d'arrivee de la demie-ellipse avec la mire"))
...
L11 D AFFI^%SDUMEN(0,0,$$^%QZCHW("Pointez l'extremite du rayon sur l'axe avec la mire"))
L2 D ^%PBPZGRA S R=$$LOC^%PBMFN(.XR,.YR)
 I R=0,OBLI G L2
 D EFFBAS^%SDUMEN
 I R=0 D EFF3 S OK=0 G FIN
 S D=((XR*A)+(YR*B))+C I $$SQROOT^%PBCACV(D*D)>5 D ^%VSQUEAK G L2
 I ((XR-$P((XD+XA)/2,".",1))<5)&((YR-$P((YD+YA)/2,".",1))<5) G L11
 D DROITE^%PBMFN(XHD,YHD,XHA,YHA,1,"M",VIS)
 I B'=0 S YR=$P((((-(A))*XR)-C)/B,".",1)
 E  S XR=$P((((-(B))*YR)-C)/A,".",1)
 D CAL,CAL2
 D DEMIE^%PBMFN(XC,YC,PR,GR,I,0,TYPL,VIS) H 2
FIN Q
 
AXE D HORTO^%PBMFN(XD,YD,XA,YA,.XHD,.YHD,.XHA,.YHA,.A,.B,.C)
 D DROITE^%PBMFN(XHD,YHD,XHA,YHA,0,"M",VIS)
 Q
...
EFF(XD,YD,XA,YA,XR,YR,TYPL,VIS) 
 N A,B,C,XHD,YHD,XHA,YHA,PR,GR,XC,YC
 S TYPL=$$TYPL^%SDEDEF($$ZSUBST^%QZCHSUB(TYPL,"""",""))
 D HORTO^%PBMFN(XD,YD,XA,YA,.XHD,.YHD,.XHA,.YHA,.A,.B,.C)
 D CAL,CAL2
 D ^%PBPZGRA,EFF1,EFF2,DEMIE^%PBMFN(XC,YC,PR,GR,I,1,TYPL,VIS),EFF^%PBP5EC(0)
 Q
