...
TKDEV2 ;
 ...
 S (TT,ZF)="" G:$D(^TK("TERMST"))=0 BEG S:$D(TK(1))=0 TK(1)=0 S TT=^TK("TERMST") X ^TK("TERM-ON")
...
BEG W $$^%QZCHW("Press RETURN to skip over a function."),!,!
 F C=1:1 S B=$P($T(FUNC+C),";",2,9) Q:B=""  I H[$P(B,";") S SB=$P(B,";",3),A=$P(B,";",2),T=$S(A=3:"HARD COPY DEVICES",1:0),O=$S($D(^TK(T,DV,SB)):^(SB),1:"") D ENTER
 D LAST F I=0:1:5 K ^TK(T,DV,I)
 F I=0:1:M S ^(I)="S "_$E(M(I),2,255)
 X:TT'="" ^TK("TERM-OFF") K:$D(TK)=10 TK G OK^TKDEV
ERASE X "F J=J:-1:1 W $C(8,32,8)" Q
LAST S A="",SB="Z" F I="ROF","UF","HI" S A=A_^TK(0,DV,I)
 S ^TK(0,DV,SB)=A S:DV["VT" ^(SB)=$C(27,91,59,49,109) S Y="$C(" F J=1:1:$L(A) S:J>1 Y=Y_"," S Y=Y_$A(A,J)
 S Y=Y_")" D S1 Q
...
S1 D:A=2 S2 S:Y="" Y="""""" S:($L(M(M))+$L(Y))>240 M=M+1,M(M)="" S M(M)=M(M)_",TK("""_SB_""")="_Y Q
