...
;TKNOTE^INT^1^59547,74869^0
TKNOTE ;
 
 Q
SETUP D:'($D(TK)) INIT^TKDEV S TK("OS")=^TK("OS"),^TKNOTE=$P($T(BEG)," ",2,99) X:TK("OS")="DTM" "S ^TKNOTE=$XECUTE(^TKNOTE)"
 F I=1:1 S A=$T(BEG+I) Q:A=""  S ^TKNOTE($P(A," "))=$P(A," ",2,99) X:TK("OS")="DTM" "S A=$P(A,"" ""),^(A)=$XECUTE(^(A))"
 G ^TKNOTE1
