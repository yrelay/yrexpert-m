...
TLTRDI2 ;
 ...
 S @MENU@(1,"COM")=$$^%QZCHW("Impression apres selection des dialogues")
 S @MENU@(2)=$$^%QZCHW("Tous")_"^TOUS"
 S @MENU@(2,"COM")=$$^%QZCHW("Impression de tous les dialogues")
ACTU D CLEPAG^%VVIDEO
 D ^%VZCD(0,39,0,4,1,0,$$^%QZCHW("IMPRESSION DES DIALOGUES"))
...
SELEC 
 K @(ENSEM)
 D INIT^%QUCHOIP("^[QUI]TLDIAL",1,"",0,5,80,17)
 D AFF^%QUCHOIP
 D ^%QUCHOYP(ENSEM)
...
IMPRI(LISTE) 
 ...
 S I=""
 F %I=0:0 S I=$O(@LISTE@(I)) Q:(I="")!(I="z")  D POCLEPA^%VVIDEO W I D IMPR^%TLTRDI3(I)
