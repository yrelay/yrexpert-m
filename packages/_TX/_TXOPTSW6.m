...
;%TXOPTSW6^INT^1^59547,74032^0
%TXOPTSW6(etuatc,CONTRAINTE,ENSEMBLE) 
 ...
 S STO="STOCON",VAL="VALCON" D INIPA S NBC=TAIPAGE+1
...
INICONF 
 ...
 Q
INIVAR 
 F i=1:1:2 S v=$P($T(VAR+i),";;",2),v1=$P(v,"~",1),v2=$P(v,"~",2),VAR(i)=v1 S:v2["$$" @("v2="_v2) S:'($D(@GSUIV@(VAR(i)))) @GSUIV@(VAR(i))=v2
 F j=1:1:TAIPAGE S i=2+j,v="Periode ",v1=((PAGE-1)*TAIPAGE)+j,v3=v_v1,VAR(i)=v3 D defaut(v1,i,GSUIV)
 Q
defaut(POSIR,POSIPA,G) 
...
INIPA S PAGEMAX=PMAX,TAIPAGE=10
 S PAGE=1,PAGEMAX=(PMAX\TAIPAGE)+$S((PMAX#TAIPAGE)=0:0,1:1)
 Q
INIV(GSUIV) 
 D INIPA
 S:'($D(@GSUIV@("Valeur Standard"))) @GSUIV@("Valeur Standard")=0
 F PAGE=1:1:PAGEMAX D INIVAR
 Q
