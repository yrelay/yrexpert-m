...
;%TXP1R^INT^1^59547,74032^0
%TXP1R(pb,apb,ATELIER,ARTICLE,PMAX,dmd,STOCKINI,VISU) ;;04:44 PM  3 May 1993; ; 25 Jun 93  5:33 PM
...
optq n a,fmi,fmx,count,mlr,MCOUNT
 s MCOUNT=100
 ...
 s f=VLIMI,qopt=-1,fmx=f,a(4)=f,a(5)=f,mlr=1,count=0,qoptlast=0
 ...
 f qx=QMIN:1:QMAX s count=count+1 q:(count>MCOUNT)&(qopt'<0)  s q=qx*qlot,fmi=$$f(t+1,(r+q)-d,PB),Cqd=$$COUT(t,q,r,d)+fmi d print i f>Cqd s f=Cqd,qopt=q,fmx=fmi i qopt'=qoptlast s count=0,qoptlast=qopt
...
f(t,r,pb) 
 ...
 s r1=(r\lot)*lot i '($d(@PRD@(t,r1))),$$^%TXP2RB(t,r,PB)>VLIMI q VLIMI
...
V(pb,apb,ATELIER,ARTICLE,PMAX,demd,STOCKINI,VISU) 
 ...
 d ^%TXP1R(pb,apb,ATELIER,ARTICLE,PMAX,demd,STOCKINI,(VISU#2)=1)
 
 i '($d(@PRD@(tdeb))) s %V=VLIMI g FINV
 s rdeb="",rdeb=$o(@PRD@(tdeb,rdeb)),il=@PRD@(tdeb,rdeb) i @prd@(il,9)<VLIMI w:(VISU#2)=1 !,PB," ","r init= ",rdeb," v ",@prd@(il,9),! s %V=@prd@(il,9),@prd@("NIVEAU.INITIAL")=rdeb
 i (VISU#2)=1 r *uuu w !,!,!
FINV q %V
diag(tdeb,AVORT) 
 s AVORT=0
