...
REFUL D ^%VSQUEAK,POCLEPA^%VVIDEO W "Informations trop abondantes : veuillez abreger les champs " H 2
 ...
 H 2 D ^%VAFIGRI,^%VAFISCR S IC=1,ABENDSCR=0 D ^%VKMODIS G AUTOSTOP:ABENDSCR=0 Q
REFUS D ^%VSQUEAK,POCLEPA^%VVIDEO W "Il faut obligatoirement une information dans  ",^%SCRE(SCR,IC,1) H 2 D ^%VAFIGRI,^%VAFISCR S ABENDSCR=0 D LOOP^%VKMODIS G AUTOSTOP:ABENDSCR=0 Q
