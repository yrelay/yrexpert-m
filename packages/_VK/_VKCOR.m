...
LOOP K PATOUCH S IFLAG=0 D @ROUTI G CHAN:IFLAG'=0 D VERIF:SCR="MESSA" G FIN:ABENDSCR=1 F %U=1:1:^%SCRE(SCR) S PATOUCH(%U)=$S($D(^%SCRE(SCR,%U,"PATOUCH")):1,1:"")
 D ^%VAFIGRI,^%VAFISCR
...
LOOP2 S %AJCMS="Cons/Mod/Supp"
 D ^%VATYPA R *X1:^TOZE($I,"ATTENTE") S:X1=-1 X1=1
 ;;DTM
 G:$$FLECHE^%INCCLAV(X1,.X1,IFLAG,%AJCMS) REFUS
 D TOUCHE^%INCCLAV(X1,.X1)
 ;;FINDTM
 G IMPR:X1=10,FIN:X1=6,FIN:X1<2 S:X1>91 X1=X1-32
 S ISN=$S(X1=43:"PLUS",X1="61":"PLUS",X1=45:"MOINS",X1=77:"M",X1=83:"S",X1=67:"C",X1=65:"A",X1=68:"D",1:"REFUS"),PADMESSA="                                                                                " D:ISN'="REFUS" PAD^%VZEATT G @ISN
...
D D ACCOR G:'(ACCOR) LOOP1 S OAJCMS=$S($D(%AJCMS):%AJCMS,1:""),%AJCMS="Duplication",ABENDSCR=0
 D POCLEPA^%VVIDEO,^%VSQUEAK W "N'oubliez pas de changer les champs cles" H 2 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^%VKMODIS S %AJCMS=OAJCMS G LOOP1:ABENDSCR=1 D ^%VKAUST S ^PRESCR($I,SCR,%NBPVU)=NOMTAB_"("_$P(NOM,"(",2,599),%PCOUR=%NBPVU,%NBPVU=%NBPVU+1 D NEWOLD G LOOP1
M D ACCOR G:'(ACCOR) LOOP1 S ABENDSCR=0,%AJCMS="Modification" D ^%VATYPA,^%VKMODIS G LOOP1:ABENDSCR=1 K @(OLDNOM),@(OLDFULL) D ^%VKAUST S ^PRESCR($I,SCR,%PCOUR)=NOMTAB_"("_$P(NOM,"(",2,599) D NEWOLD G LOOP1
S D ACCOR G:'(ACCOR) LOOP1 D ^%VSQUEAK S PADMESSA="                          **** confirmer la suppression ****" D PAD^%VZEATT R *X1:^TOZE($I,"ATTENTE") G LOOP1:X1'=83 K @(OLDNOM),@(OLDFULL),^PRESCR($I,SCR,%PCOUR)
 D:$D(%MOINS) @%MOINS
