...
;%VKCORQ^INT^1^59547,74035^0
CMSCORQ ;;08:39 AM  20 Apr 1989
 
 
 K ^PRESCR($I,SCR) S (%PCOUR,%NBPVU)=1 S:'($D(^TOZE($I,"FULL"))) ^TOZE($I,"FULL")=0
 I ^TOZE($I,"FULL")=1 D ^%VTNAMFU S NOMTAB=$S(%NFU="":NOMTAB,1:%NFU)
LOOP S IFLAG=0 D @ROUTI G CHAN:IFLAG'=0 D VERIF:SCR="MESSA" G FIN:ABENDSCR=1
 F %U=1:1:^%SCRE(SCR) S PATOUCH(%U)=$S($D(^%SCRE(SCR,%U,"PATOUCH")):1,1:"")
 D ^%VAFIGRS
 S DY=0,DX=68 X XY W "          " X XY W "Page : ",%NBPVU S ^PRESCR($I,SCR,%NBPVU)=%NOMC,%PCOUR=%NBPVU,%NBPVU=%NBPVU+1
LOOP1 S ABENDSCR=0 D POCLEPA^%VVIDEO W $$^%QZCHW("(C)onsulter,(M)odifier,(S)upprimer,(A)jouter,(D)upliquer,(+),(-),Page ")
LOOP2 S %AJCMS="Cons/Mod/Supp" D ^%VATYPA R *X1
 ;;DTM
 G:$$FLECHE^%INCCLAV(X1,.X1,DY,DX) REFUS
 D TOUCHE^%INCCLAV(X1,.X1)
 ;;FINDTM
 G IMPR:X1=10,FIN:X1=6,FIN:X1<2 S:X1>91 X1=X1-32 S ISN=$S(X1=43:"PLUS",X1="61":"PLUS",X1=45:"MOINS",X1=77:"M",X1=83:"S",X1=67:"C",X1=65:"A",X1=68:"D",1:"REFUS"),PADMESSA="                                                                                " D:ISN'="REFUS" PAD^%VZEATT G @ISN
...
D D ACCOR G:'(ACCOR) LOOP1 S OAJCMS=$S($D(%AJCMS):%AJCMS,1:""),%AJCMS="Duplication",ABENDSCR=0
 D POCLEPA^%VVIDEO,^%VSQUEAK W "N'oubliez pas de changer les champs cles" H 2 D CLEPAG^%VVIDEO,^%VAFIGRI,^%VAFISCR,^%VKMODIS S %AJCMS=OAJCMS G LOOP1:ABENDSCR=1 D ^%VKAUSTQ S ^PRESCR($I,SCR,%NBPVU)=NOMTAB_"("_$P(NOM,"(",2,599),%PCOUR=%NBPVU,%NBPVU=%NBPVU+1 D NEWOLD G LOOP1
M D ACCOR G:'(ACCOR) LOOP1 S ABENDSCR=0,%AJCMS="Modification" D ^%VATYPA,^%VKMODIS G LOOP1:ABENDSCR=1 K @(OLDNOM),@(OLDFULL) D ^%QUERKILL,^%VKAUSTQ S ^PRESCR($I,SCR,%PCOUR)=NOMTAB_"("_$P(NOM,"(",2,599) D NEWOLD G LOOP1
S D ACCOR G:'(ACCOR) LOOP1 D ^%VSQUEAK S PADMESSA="                          **** confirmer la suppression ****" D PAD^%VZEATT R *X1 G LOOP1:X1'=83 D OABRES^%VKMODIS,^%QUERKILL K @(OLDNOM),@(OLDFULL),^PRESCR($I,SCR,%PCOUR) D CLEPAG^%VVIDEO G LOOP1
