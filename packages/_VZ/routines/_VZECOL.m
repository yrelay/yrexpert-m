...
;%VZECOL^INT^1^59547,74037^0
HXYCOL(X,Y,H,L,LC,E,CL) 
 ...
 N NOCOL,NOLIN,LIN,COL,LIMLIN,LIMCOL,ESPREST,LCVAR,DX,DY
 S NOCOL=$P(@CL,"^",1)
 S NOLIN=$P(@CL,"^",2)
 S DY=Y,DX=X X XY
 ...
 Q
BOUC 
 F COL=1:1:LIMCOL S CH=$S($D(@CL@(COL,LIN)):@CL@(COL,LIN),1:"") W $E(CH,1,LCVAR),$J(" ",E) I $L(CH)<LCVAR W $J(" ",LCVAR-$L(CH))
...
TEST N CL,LIN,COL
 K CL1 S CL1="5^5"
 F LIN=1:1:5 F COL=1:1:5 S CL1(COL,LIN)=$$LINA(60,65)
 D HXYCOL(5,5,10,65,10,2,"CL1"),^%VZEAVT("") Q
