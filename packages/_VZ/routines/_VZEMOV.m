...
;%VZEMOV^INT^1^59547,74037^0
HXYMOV(X,Y,C,L,EX,EY,GL,NC,NL) 
 ...
 N CH,I,LARGE,HAUT,PRECOL,PRECOL2,PRECLIN,PRECLIN2,FLAG,CENTRAGE
 S FLAG=0
 S LARGE=(C*EX)-20,HAUT=L*EY
 S PRECOL=1,PRECLIN=1
 S (NC,NL)=0
 D PROMPT
...
PROMPT 
 ...
 W $E(CH,1,LARGE)
 D NORM^%VVIDEO
 S DX=X+(EX*(PRECOL-1)),DY=Y+(EY*(PRECLIN-1)) X XY
 Q
CLRPROMP 
 S DX=X+15,DY=HAUT+Y
 X XY W $J(" ",LARGE+1)
...
TEST N CL,LIN,COL K CL
 S CL="5^5"
 F LIN=1:1:5 F COL=1:1:5 S CL(COL,LIN)=$$LINA(0,80)
 D ^%VZECOL(5,5,10,60,9,2,"CL"),CARSP^%VVIDEO(4,4,8,60,"BLD")
 S XER=$$^%VZEMOV(5,5,5,5,11,1,.CL,.NC,.NL)
 Q
