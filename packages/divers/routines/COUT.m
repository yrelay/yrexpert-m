...
RETERR S %RESULTAT="1???",$ZT=^TOZE($I,"A") Q:%ABEND=1  K ITEX S ITEX=$P(EXPFONC,"/",2,500),IIT=1,(%FPH,%FOP)="" F IT="%FPH","%FOP" I $P(ITEX,"/",IIT)'="" D TRAIT S IIT=IIT+1
 G:((%FPH="")!%FOP)="" FIN
PH G:'($D(^PHASADM($I,NUFA,TWREF,"PHAS","OK",%FPH,%FOP))) PH1 S %FOG=^PHASADM($I,NUFA,TWREF,"PHAS","OK",%FPH,%FOP) I '($D(^[QUI]COUT(NUFA,TWREF,%FOP_"/"_%FOG))) S %PAAAF=0 G FIN
 S %RESULTAT=^[QUI]COUT(NUFA,TWREF,%FOP_"/"_%FOG) G FIN
