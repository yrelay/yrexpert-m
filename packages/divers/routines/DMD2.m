...
;DMD2^INT^1^59547,74866^0
DMD2 ;
 
 S %RESULTAT=0 I BATCH=1 S %ABEND=1 G FIN
...
NOTIL S DE=6,FI=13 D ^%ABNETTO S DY=6,DX=5 X XY W "Le SF exige de demander la valeur de ",%PAR,IND
 I $D(^V($I,YA,%PAR_IND)) S DY=DY+2,DX=5 X XY W "Valeur actuelle de ",%PAR,IND," : ",^V($I,YA,%PAR_IND)
 I $D(^VAL($I)) S DY=DY+1,DX=5 X XY W "Valeurs legales : ",! S %ARG=-1 F %FF=1:1 S %ARG=$N(^VAL($I,%ARG)) Q:%ARG=-1  W ?(((%FF-1)#4)*20)+5,%ARG I (%FF#4)=0 W !
QUES S DE=13,FI=13 D ^%ABNETTO W "Valeur de ",%PAR,IND S DX=$X D ^%VLEC I (CTRLA=1)!((CTRLF=1)!(Y1="")) G QUES
 I $D(^VAL($I)),'($D(^VAL($I,Y1))) D ^%VSQUEAK G QUES
 S ^V($I,YA,%PAR)=Y1,%RESULTAT=Y1,@%TABRA@(%PAR)="IMPLICIMODIFICATION",@%TABTV@(%PAR)=Y1,VPA=Y1 K OAPARA S:$D(PARA) OAPARA=PARA
