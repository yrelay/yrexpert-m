...
;DTLOGL^INT^1^59547,74866^0
ZTRANSG ;
 
 K ^TRANS($J) D ^%ZD,INT^%T S ^TRANS($J)=%DAT_"|"_%TIM K ^GLOB($J) S %NG=1 D %UCI^%QCSDIR S %CPT=0
 W !,"Tous les globals (O/N) : " R *REP S REP=$C(REP) Q:(REP'="O")&(REP'="N")  G:REP="N" LEC W "UI   OK !" D ^%GLO S %G=0 F %U=1:1 S %G=$N(^UTILITY("GLO",%G)) Q:%G=-1  S ^GLOB($J,%U)=%G
 S %NG=$P(^UTILITY("GLO",0),"^",1)+1 G COPJ
LEC W !,"Global : ^" R %G G:$A(%G)<32 COPI S ^GLOB($J,%NG)=%G,%NG=%NG+1 G LEC
COPI I '($D(^GLOB($J))) G FIN
COPJ W !,!,%NG-1," Global",$S(%NG>2:"s",1:"")," a copier",!
